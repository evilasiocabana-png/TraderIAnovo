...
def is_active_operational_model(value: object) -> bool:
    ...
    if normalized.startswith("MODELO_26_XAU_M5_SMART_MONEY"):
        return True
    if normalized.startswith("MODELO_27_ESPELHO_M26"):
        return True
    if normalized.startswith("MODELO_28_PATTERN_MINER_SHADOW"):
        return True
...
def is_shadow_operational_model(value: object) -> bool:
    """Return True only for models allowed to observe but never send orders."""

    normalized = str(value or "").strip().upper()
    return operational_model_number(normalized) in SHADOW_OPERATIONAL_MODEL_NUMBERS
