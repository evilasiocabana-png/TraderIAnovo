# MISSION_TIA-019_AUTORIZAR_TIME_STOP_DEMO

## Objetivo

Criar uma camada segura de pre-autorizacao para `TIME_STOP` em modo demo, sem
ligar execucao operacional nesta fase.

## Guardrails

- Nao executar ordem.
- Nao fechar posicao.
- Nao alterar o Provider Demo operacional.
- Nao permitir `dynamic_exit_allowed_to_execute_demo=true`.
- Nao recalcular Lab pesado no ciclo leve Forex.
- Nao autorizar politica diferente de `TIME_STOP`.

## Resultado esperado

Autorizador read-only, testes e rastreabilidade para que uma etapa futura possa
ligar execucao demo de forma explicita e reversivel.
