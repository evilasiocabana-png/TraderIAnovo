"""Testes de integracao da Alpha201 com o ecossistema de Research."""
...
from research.alpha201.alpha201_experiment import Alpha201Experiment
from research.alpha201.alpha201_research_integration import (
    Alpha201ResearchIntegration,
)
...
class Alpha201ResearchIntegrationTest(unittest.TestCase):
    """Valida reutilizacao integral da arquitetura existente pela Alpha201."""

    def test_research_pipeline_processa_alpha201_com_engines_existentes(self) -> None:
        calls: list[str] = []
        integration = Alpha201ResearchIntegration(
    ...
    def test_research_runner_executa_alpha201_sem_alterar_runner(self) -> None:
        calls: list[str] = []
        ...
        result = Alpha201ResearchIntegration().run_with_runner(
            runner,
    ...
    def test_validation_reproducibility_e_campaigns_reutilizam_componentes(self) -> None:
        ...
        self.assertEqual(fingerprint.experiment_id, "exp-alpha201-001")
        self.assertTrue(reproducibility.is_reproducible)
        self.assertEqual(campaign_experiments[0].alpha_id, "Alpha201")
        self.assertGreaterEqual(len(validation.passed_rules), 1)

    def test_alpha201_nao_cria_engines_de_research_proprios(self) -> None:
        alpha201_files = [
            path
            for path in Path("research/alpha201").glob("*.py")
            if path.name != "__init__.py"
        ]
        forbidden_class_names = (
            "Alpha201MetricsEngine",
            "Alpha201ProfitEngine",
            "Alpha201DrawdownEngine",
            "Alpha201WinRateEngine",
            "Alpha201ProfitFactorEngine",
            "Alpha201ExpectancyEngine",
        )
        forbidden_files = [
            path
            for path in alpha201_files
    ...
    def test_integracao_nao_acopla_labs_operacionais_ou_infraestrutura(self) -> None:
        path = Path("research/alpha201/alpha201_research_integration.py")
        forbidden_imports = {
    ...
    def test_research_pipeline_permanece_sem_acoplamento_alpha201(self) -> None:
        source = read_source(Path("research/research_pipeline.py"))

        self.assertNotIn("Alpha201", source)

    def _experiment(self) -> Alpha201Experiment:
        return Alpha201Experiment(
            config=self._config(),
            candles=(
    ...
    def _snapshot(self) -> ResearchSnapshot:
        return ResearchSnapshot(
            snapshot_id="snapshot-alpha201-001",
            experiment_id="exp-alpha201-001",
            alpha_id="Alpha201",
            alpha_version="1.0",
            configuration_version="cfg-201",
            ...
            metadata={"parameters": {"risk_profile": "position-research"}},
        )

    def _campaign(self) -> ResearchCampaign:
        return ResearchCampaign(
            campaign_id="campaign-alpha201",
            name="Alpha201 Research Campaign",
            description="Campanha declarativa da Alpha201.",
            alpha_id="Alpha201",
            objective="Validar Alpha201 em pesquisa.",
            status="PENDING",
            created_at="2026-06-27T10:00:00",
            created_by="CTO",
    ...
    def _experiment_definition(self) -> ExperimentDefinition:
        return ExperimentDefinition(
            experiment_id="template-alpha201",
            alpha_id="Alpha201",
            alpha_version="1.0",
            configuration_version="cfg-201",
            replay_period="2026-01",
    ...
    def _execution_result(self):
        ...
        return Alpha201ResearchIntegration().run_with_runner(
