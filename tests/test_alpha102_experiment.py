"""Testes do experimento controlado da Alpha102."""
...
from research.alpha102.alpha102_experiment import (
    Alpha102Experiment,
    Alpha102ExperimentResult,
)
...
class Alpha102ExperimentTest(unittest.TestCase):
    """Valida execucao estrutural da Alpha102 sem metricas financeiras."""

    def test_resultado_e_dataclass_frozen(self) -> None:
        self.assertTrue(is_dataclass(Alpha102ExperimentResult))
        self.assertTrue(Alpha102ExperimentResult.__dataclass_params__.frozen)

    def test_resultado_e_imutavel(self) -> None:
        result = Alpha102ExperimentResult(
            total_candles=0,
            total_signals=0,
            total_buy=0,
            total_sell=0,
    ...
    def test_experiment_executa_replay_e_consolida_sinais(self) -> None:
        ...
        self.assertIsInstance(result, Alpha102ExperimentResult)
        ...
        self.assertEqual(
            [signal.decision for signal in result.signals],
            ["BUY", "SELL", "WAIT"],
        )
    ...
    def test_experiment_default_gera_features_de_pullback_momentum(self) -> None:
        experiment = Alpha102Experiment(
            config=self._config(),
            candles=self._candles(),
            strategy=_InspectingStrategy(),
    ...
    def test_experiment_nao_calcula_metricas_financeiras(self) -> None:
        source = read_source(Path("research/alpha102/alpha102_experiment.py"))
        forbidden_fragments = (
            "gross_profit",
            "gross_loss",
            "net_profit",
    ...
    def test_experiment_permanece_desacoplado_de_operacao(self) -> None:
        path = Path("research/alpha102/alpha102_experiment.py")
        ...
        forbidden_imports = {
            ...
            "research.alpha101.alpha101_experiment",
            "research.alpha201.alpha201_experiment",
            "research.alpha301.alpha301_experiment",
            "risk.risk_engine",
            "broker",
            "mt5",
            "MetaTrader5",
    ...
    def _candles(self) -> tuple[Candle, ...]:
        return (
            Candle("2026-06-28T10:00:00-03:00", 5500.0, 5520.0, 5480.0, 5525.0, 2000),
            Candle("2026-06-28T10:01:00-03:00", 5525.0, 5530.0, 5490.0, 5510.0, 2100),
            Candle("2026-06-28T10:02:00-03:00", 5510.0, 5510.0, 5490.0, 5510.0, 1800),
        )
    ...
    def _feature_report(
        ...
        return FeatureReport(
            ...
            calculated_values={
                ...
                "recovery_confirmation": candle.fechamento - candle.abertura,
