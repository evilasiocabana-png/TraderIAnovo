"""Testes do Data Readiness Gate para datasets Parquet."""
...
class ParquetReadinessGateTest(unittest.TestCase):
    """Valida que Parquet usa o mesmo readiness gate dos demais providers."""

    def test_parquet_sem_validacao_retorna_not_validated_e_bloqueia_fluxos(
        self,
    ) -> None:
        logger = InMemoryDataReadinessGateLogger()
        service = self._service_for_parquet_gate(
    ...
    def test_parquet_invalido_retorna_not_ready_e_bloqueia_fluxos(self) -> None:
        ...
        service = self._service_for_parquet_gate(repository, logger)
    ...
    def test_parquet_aprovado_libera_replay_e_research(self) -> None:
        repository = InMemoryHistoricalDatasetQualityRepository()
        logger = InMemoryDataReadinessGateLogger()
        ...
        self.assertEqual(experiment.experiment_name, "wdo_parquet_gate")
        self.assertEqual(
            [log.decision for log in service.list_data_readiness_gate_logs()],
            ["ALLOWED", "ALLOWED"],
        )
    ...
    def test_camadas_superiores_nao_acessam_parquet_diretamente(self) -> None:
        forbidden_terms = (
            "ParquetHistoricalDataAdapter",
    ...
    def _service_for_parquet_gate(
        ...
        service.select_historical_dataset("wdo_parquet_gate")
        return service
