"""Testes de integracao da Alpha301 com o ecossistema de Research."""
...
from research.alpha301.alpha301_research_integration import (
    Alpha301ResearchIntegration,
)
...
class Alpha301ResearchIntegrationTest(unittest.TestCase):
    """Valida reutilizacao integral da arquitetura existente pela Alpha301."""

    def test_research_pipeline_processa_alpha301_com_engines_existentes(self) -> None:
        calls: list[str] = []
        integration = Alpha301ResearchIntegration(
    ...
    def test_research_runner_executa_alpha301_sem_alterar_runner(self) -> None:
        calls: list[str] = []
        ...
        result = Alpha301ResearchIntegration().run_with_runner(
            runner,
    ...
    def test_validation_reproducibility_e_campaigns_reutilizam_componentes(self) -> None:
        ...
        self.assertEqual(fingerprint.experiment_id, "exp-alpha301-001")
        self.assertTrue(reproducibility.is_reproducible)
        self.assertEqual(campaign_experiments[0].alpha_id, "Alpha301")
        self.assertGreaterEqual(len(validation.passed_rules), 1)

    def test_knowledge_base_reutiliza_componentes_existentes(self) -> None:
        artifact = self._artifact()
        integration = Alpha301ResearchIntegration(
            knowledge_extractor=_SpyKnowledgeExtractor((artifact,)),
        )

        extracted = integration.extract_knowledge(object(), object(), object())
        saved = integration.save_knowledge(extracted[0])
        indexed = integration.index_knowledge(extracted)
        search_result = integration.search_knowledge("relative value")

        self.assertEqual(extracted, (artifact,))
        self.assertIs(saved, artifact)
        self.assertEqual(indexed, (artifact,))
        self.assertEqual(search_result, (artifact,))

    def test_alpha301_nao_cria_engines_de_research_proprios(self) -> None:
        alpha301_files = [
            path
            for path in Path("research/alpha301").glob("*.py")
            if path.name != "__init__.py"
        ]
        forbidden_class_names = (
            "Alpha301MetricsEngine",
            "Alpha301ProfitEngine",
            "Alpha301DrawdownEngine",
            "Alpha301WinRateEngine",
            "Alpha301ProfitFactorEngine",
            "Alpha301ExpectancyEngine",
        )
        forbidden_files = [
            path
            for path in alpha301_files
    ...
    def test_integracao_nao_acopla_labs_operacionais_ou_infraestrutura(self) -> None:
        path = Path("research/alpha301/alpha301_research_integration.py")
        forbidden_imports = {
    ...
    def test_research_pipeline_permanece_sem_acoplamento_alpha301(self) -> None:
        source = read_source(Path("research/research_pipeline.py"))

        self.assertNotIn("Alpha301", source)

    def _experiment(self) -> Alpha301Experiment:
        return Alpha301Experiment(
            config=self._config(),
            candles=(
    ...
    def _snapshot(self) -> ResearchSnapshot:
        return ResearchSnapshot(
            snapshot_id="snapshot-alpha301-001",
            experiment_id="exp-alpha301-001",
            alpha_id="Alpha301",
            alpha_version="1.0",
            configuration_version="cfg-301",
            ...
            metadata={"parameters": {"risk_profile": "long-short-research"}},
        )

    def _campaign(self) -> ResearchCampaign:
        return ResearchCampaign(
            campaign_id="campaign-alpha301",
            name="Alpha301 Research Campaign",
            description="Campanha declarativa da Alpha301.",
            alpha_id="Alpha301",
            objective="Validar Alpha301 em pesquisa.",
            status="PENDING",
            created_at="2026-06-27T10:00:00",
            created_by="CTO",
    ...
    def _experiment_definition(self) -> ExperimentDefinition:
        return ExperimentDefinition(
            experiment_id="template-alpha301",
            alpha_id="Alpha301",
            alpha_version="1.0",
            configuration_version="cfg-301",
            replay_period="2026-01",
    ...
    def _execution_result(self):
        ...
        return Alpha301ResearchIntegration().run_with_runner(
    ...
    def _artifact(self) -> KnowledgeArtifact:
        return KnowledgeArtifact(
            artifact_id="artifact-alpha301-relative-value",
            alpha_id="Alpha301",
            research_id="research-alpha301",
            campaign_id="campaign-alpha301",
            hypothesis="relative value",
            conclusion="Alpha301 preserva relacao estatistica.",
            evidence=("relative value aprovado",),
            confidence=0.9,
            created_at=datetime(2026, 6, 27, 10, 0, 0),
            metadata={"regime": "relative value"},
        )
...
class _SpyKnowledgeExtractor:
    def __init__(self, artifacts: tuple[KnowledgeArtifact, ...]) -> None:
        self.artifacts = artifacts

    def extract(
        self,
        research_report: object,
        campaign_report: object,
