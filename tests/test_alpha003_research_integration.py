"""Testes de integracao da Alpha003 com o ecossistema de Research."""
...
from research.alpha003.alpha003_research_integration import (
    Alpha003ResearchIntegration,
)
...
class Alpha003ResearchIntegrationTest(unittest.TestCase):
    """Valida reutilizacao integral da arquitetura existente pela Alpha003."""

    def test_research_pipeline_processa_alpha003_com_engines_existentes(self) -> None:
        calls: list[str] = []
        integration = Alpha003ResearchIntegration(
    ...
    def test_research_runner_executa_alpha003_sem_alterar_runner(self) -> None:
        calls: list[str] = []
        ...
        result = Alpha003ResearchIntegration().run_with_runner(
            runner,
    ...
    def test_validation_reproducibility_e_campaigns_reutilizam_componentes(self) -> None:
        ...
        self.assertEqual(fingerprint.experiment_id, "exp-alpha003-001")
        self.assertTrue(reproducibility.is_reproducible)
        self.assertEqual(campaign_experiments[0].alpha_id, "Alpha003")
        self.assertGreaterEqual(len(validation.passed_rules), 1)

    def test_alpha003_nao_cria_engines_de_research_proprios(self) -> None:
        alpha003_files = [
            path
            for path in Path("research/alpha003").glob("*.py")
            if path.name != "__init__.py"
        ]
        forbidden_class_names = (
            "Alpha003MetricsEngine",
            "Alpha003ProfitEngine",
            "Alpha003DrawdownEngine",
            "Alpha003WinRateEngine",
            "Alpha003ProfitFactorEngine",
            "Alpha003ExpectancyEngine",
        )
        forbidden_files = [
            path
            for path in alpha003_files
    ...
    def test_integracao_nao_acopla_labs_operacionais_ou_infraestrutura(self) -> None:
        path = Path("research/alpha003/alpha003_research_integration.py")
        forbidden_imports = {
    ...
    def _experiment(self) -> Alpha003Experiment:
        return Alpha003Experiment(
            config=self._config(),
            candles=(
                Candle("2026-06-27T10:00:00-03:00", 5500.0, 5520.0, 5480.0, 5525.0, 2000),
                Candle("2026-06-27T10:01:00-03:00", 5500.0, 5530.0, 5490.0, 5485.0, 2100),
            ),
        )
    ...
    def _snapshot(self) -> ResearchSnapshot:
        return ResearchSnapshot(
            snapshot_id="snapshot-alpha003-001",
            experiment_id="exp-alpha003-001",
            alpha_id="Alpha003",
            alpha_version="1.0",
            ...
            metadata={"parameters": {"risk_profile": "research-only"}},
        )

    def _campaign(self) -> ResearchCampaign:
        return ResearchCampaign(
            campaign_id="campaign-alpha003",
            name="Alpha003 Research Campaign",
            description="Campanha declarativa da Alpha003.",
            alpha_id="Alpha003",
            objective="Validar Alpha003 em pesquisa.",
            status="PENDING",
            created_at="2026-06-27T10:00:00",
            created_by="CTO",
    ...
    def _experiment_definition(self) -> ExperimentDefinition:
        return ExperimentDefinition(
            experiment_id="template-alpha003",
            alpha_id="Alpha003",
            alpha_version="1.0",
            configuration_version="cfg-1",
            replay_period="2026-01",
    ...
    def _execution_result(self):
        ...
        return Alpha003ResearchIntegration().run_with_runner(
