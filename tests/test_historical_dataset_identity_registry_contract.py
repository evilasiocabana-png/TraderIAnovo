"""Contrato historico de registry de identidades."""
...
from historical.contracts.historical_dataset_identity_registry import (
    HistoricalDatasetIdentityRegistry,
)


CONTRACT_PATH = Path("historical/contracts/historical_dataset_identity_registry.py")
REQUIRED_FIELDS = [
    "registry_id",
    "registry_name",
    "identities",
...
class HistoricalDatasetIdentityRegistryContractTest(unittest.TestCase):
    """Protege o DTO oficial de registry historico de identidades."""

    def test_contrato_importa_e_cria_com_valores_validos(self) -> None:
        registry = self._registry(identities=(self._identity(),), total_datasets=1)

        self.assertIsInstance(registry, HistoricalDatasetIdentityRegistry)
        self.assertEqual(registry.registry_id, "identity_registry_001")
        self.assertEqual(registry.identities[0].dataset_id, "WDO_1m_2025")

    def test_estado_vazio_e_permitido_quando_explicitamente_definido(self) -> None:
        registry = self._registry(identities=(), total_datasets=0)

        self.assertEqual(registry.identities, ())
        self.assertEqual(registry.total_datasets, 0)

    def test_campos_obrigatorios_estao_definidos(self) -> None:
        contract_fields = [field.name for field in fields(HistoricalDatasetIdentityRegistry)]
        ...
        for field_name in REQUIRED_FIELDS:
            with self.subTest(field=field_name):
                ...
                with self.assertRaises(TypeError):
                    HistoricalDatasetIdentityRegistry(**partial_payload)  # type: ignore[arg-type]

    def test_type_hints_estao_presentes(self) -> None:
        hints = get_type_hints(HistoricalDatasetIdentityRegistry)
        expected_types = {
            "registry_id": str,
            "registry_name": str,
    ...
    def test_identities_deve_ser_colecao_tipada(self) -> None:
        identity = self._identity()

        self.assertEqual(self._registry(identities=(identity,)).identities, (identity,))
        with self.assertRaisesRegex(TypeError, "tuple"):
            self._registry(identities=[identity])  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "HistoricalDatasetIdentity"):
            self._registry(identities=({"dataset_id": "WDO"},))  # type: ignore[arg-type]

    def test_total_datasets_deve_ser_inteiro_nao_negativo(self) -> None:
        self.assertEqual(self._registry(total_datasets=0).total_datasets, 0)
        with self.assertRaisesRegex(ValueError, "non-negative"):
            self._registry(total_datasets=-1)
        with self.assertRaisesRegex(TypeError, "int"):
            self._registry(total_datasets="0")  # type: ignore[arg-type]
    ...
    def test_nao_usa_dicionarios_livres(self) -> None:
        ...
        self.assertFalse(
            any(field.type is dict for field in fields(HistoricalDatasetIdentityRegistry))
        )
        self.assertFalse(any(isinstance(identity, dict) for identity in registry.identities))
    ...
    def test_camadas_consumidoras_permanecem_desacopladas(self) -> None:
        ...
        for path in paths:
            with self.subTest(path=str(path)):
                ...
                self.assertNotIn(
                    "historical.contracts.historical_dataset_identity_registry",
                    source,
                )
    ...
    def test_nenhum_registry_concreto_foi_criado_fora_de_contracts(self) -> None:
        ...
        forbidden_names = (
            "historical_dataset_identity_registry.py",
    ...
    def _registry(self, **overrides: object) -> HistoricalDatasetIdentityRegistry:
        payload = self._registry_payload()
        payload.update(overrides)
        return HistoricalDatasetIdentityRegistry(**payload)  # type: ignore[arg-type]

    def _registry_payload(self) -> dict[str, object]:
        return {
            "registry_id": "identity_registry_001",
            "registry_name": "Registry historico de identidades",
            "identities": (self._identity(),),
            "total_datasets": 1,
            "registry_version": "1.0",
            "generated_at": "2026-06-27T16:45:00-03:00",
        }
