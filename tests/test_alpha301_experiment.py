"""Testes do experimento controlado da Alpha301."""
...
from research.alpha301.alpha301_experiment import (
    Alpha301Experiment,
    Alpha301ExperimentResult,
)
...
class Alpha301ExperimentTest(unittest.TestCase):
    """Valida execucao estrutural da Alpha301 sem metricas financeiras."""

    def test_resultado_e_dataclass_frozen(self) -> None:
        self.assertTrue(is_dataclass(Alpha301ExperimentResult))
        self.assertTrue(Alpha301ExperimentResult.__dataclass_params__.frozen)

    def test_resultado_e_imutavel(self) -> None:
        result = Alpha301ExperimentResult(
            total_candles=0,
            total_signals=0,
            total_long=0,
            total_short=0,
    ...
    def test_experiment_executa_replay_e_consolida_sinais(self) -> None:
        experiment = Alpha301Experiment(
            ...
            strategy=_SequenceStrategy(("LONG", "SHORT", "WAIT")),
        )

        result = experiment.run()

        self.assertIsInstance(result, Alpha301ExperimentResult)
        self.assertEqual(result.total_candles, 3)
        self.assertEqual(result.total_signals, 3)
        self.assertEqual(result.total_long, 1)
        self.assertEqual(result.total_short, 1)
    ...
    def test_experiment_sem_candles_retorna_resultado_vazio(self) -> None:
        ...
        self.assertEqual(result.total_long, 0)
        self.assertEqual(result.total_short, 0)
        self.assertEqual(result.total_wait, 0)
        self.assertEqual(result.signals, ())

    def test_experiment_default_gera_features_de_long_short(self) -> None:
        experiment = Alpha301Experiment(
            config=self._config(),
            candles=self._candles(),
            strategy=_InspectingStrategy(),
    ...
    def test_experiment_nao_calcula_metricas_financeiras(self) -> None:
        source = read_source(Path("research/alpha301/alpha301_experiment.py"))
        forbidden_fragments = (
            "gross_profit",
            "gross_loss",
            "net_profit",
    ...
    def test_experiment_permanece_desacoplado_de_operacao(self) -> None:
        path = Path("research/alpha301/alpha301_experiment.py")
    ...
    def _feature_report(
        ...
        return FeatureReport(
            ...
            validation_results=(),
            calculated_values={
                "spread": candle.fechamento - candle.abertura,
                "spread_zscore": 1.2,
                "correlation": 0.9,
                "primary_volume": candle.volume,
                "secondary_volume": candle.volume,
                "spread_volatility": candle.amplitude,
