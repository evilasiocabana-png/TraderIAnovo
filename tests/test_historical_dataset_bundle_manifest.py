"""Contrato historico de manifest de bundle de datasets."""
...
from historical.contracts.historical_dataset_bundle_manifest import (
    HistoricalDatasetBundleManifest,
)
...
CONTRACT_PATH = (
    PROJECT_ROOT / "historical" / "contracts" / "historical_dataset_bundle_manifest.py"
)
REQUIRED_FIELDS = [
    "bundle_id",
    "bundle_name",
...
class HistoricalDatasetBundleManifestContractTest(unittest.TestCase):
    """Protege o DTO oficial de manifest logico de bundle historico."""

    def test_contrato_importa_e_cria_com_valores_validos(self) -> None:
        manifest = self._bundle_manifest(packages=(self._package(),), total_packages=1)

        self.assertIsInstance(manifest, HistoricalDatasetBundleManifest)
        self.assertEqual(manifest.bundle_id, "historical_bundle_001")
        self.assertEqual(manifest.packages[0].package_id, "historical_package_001")
    ...
    def test_estado_vazio_e_permitido_quando_explicitamente_definido(self) -> None:
        ...
        self.assertEqual(manifest.total_packages, 0)
        self.assertEqual(manifest.total_datasets, 0)

    def test_campos_obrigatorios_estao_definidos(self) -> None:
        contract_fields = [
            field.name for field in fields(HistoricalDatasetBundleManifest)
        ]

        self.assertEqual(contract_fields, REQUIRED_FIELDS)

    def test_campos_sao_obrigatorios_no_construtor(self) -> None:
        payload = self._bundle_manifest_payload()
        ...
        for field_name in REQUIRED_FIELDS:
            with self.subTest(field=field_name):
                ...
                with self.assertRaises(TypeError):
                    HistoricalDatasetBundleManifest(**partial_payload)  # type: ignore[arg-type]

    def test_type_hints_estao_presentes(self) -> None:
        hints = get_type_hints(HistoricalDatasetBundleManifest)
        expected_types = {
            "bundle_id": str,
            "bundle_name": str,
            "bundle_version": str,
    ...
    def test_packages_deve_ser_colecao_tipada(self) -> None:
        ...
        self.assertEqual(
            self._bundle_manifest(packages=(package,)).packages,
            (package,),
        )
        with self.assertRaisesRegex(TypeError, "tuple"):
            self._bundle_manifest(packages=[package])  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "HistoricalDatasetPackage"):
            self._bundle_manifest(packages=({"package_id": "WDO"},))  # type: ignore[arg-type]

    def test_totais_devem_ser_inteiros_nao_negativos(self) -> None:
        self.assertEqual(self._bundle_manifest(total_packages=0).total_packages, 0)
        self.assertEqual(self._bundle_manifest(total_datasets=0).total_datasets, 0)
        with self.assertRaisesRegex(ValueError, "total_packages"):
            self._bundle_manifest(total_packages=-1)
        with self.assertRaisesRegex(ValueError, "total_datasets"):
            self._bundle_manifest(total_datasets=-1)
        with self.assertRaisesRegex(TypeError, "total_packages"):
            self._bundle_manifest(total_packages="0")  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "total_datasets"):
            self._bundle_manifest(total_datasets="0")  # type: ignore[arg-type]
    ...
    def test_nao_usa_dicionarios_livres(self) -> None:
        ...
        self.assertFalse(
            any(field.type is dict for field in fields(HistoricalDatasetBundleManifest))
        )
        self.assertFalse(any(isinstance(package, dict) for package in manifest.packages))
    ...
    def test_contrato_e_dataclass_imutavel(self) -> None:
        ...
        with self.assertRaises(FrozenInstanceError):
            manifest.bundle_id = "outro"  # type: ignore[misc]
    ...
    def test_camadas_consumidoras_permanecem_desacopladas(self) -> None:
        ...
        for path in paths:
            with self.subTest(path=str(path)):
                ...
                self.assertNotIn(
                    "historical.contracts.historical_dataset_bundle_manifest",
                    source,
                )

    def test_nenhum_importer_exporter_provider_manifest_builder_ou_registry_concreto_foi_criado(
        self,
    ) -> None:
        historical_files = [
            path.name
        ...
        forbidden_names = (
            ...
            "historical_dataset_bundle_manifest_builder.py",
            "historical_dataset_bundle_manifest_factory.py",
            "historical_dataset_bundle_manifest_service.py",
        )
    ...
    def _bundle_manifest(
        ...
        **overrides: object,
        ...
        return HistoricalDatasetBundleManifest(**payload)  # type: ignore[arg-type]

    def _bundle_manifest_payload(self) -> dict[str, object]:
        return {
            ...
            "generated_at": "2026-06-27T20:30:00-03:00",
        }
    ...
    def _package(self) -> HistoricalDatasetPackage:
        return HistoricalDatasetPackage(
            ...
            manifests=(self._dataset_manifest(),),
            total_datasets=1,
            package_version="1.0",
            created_at="2026-06-27T18:45:00-03:00",
        )

    def _dataset_manifest(self) -> HistoricalDatasetManifest:
