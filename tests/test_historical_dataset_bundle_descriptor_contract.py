"""Contrato historico de descritor de bundle de datasets."""
...
from historical.contracts.historical_dataset_bundle_descriptor import (
    HistoricalDatasetBundleDescriptor,
)
...
CONTRACT_PATH = Path("historical/contracts/historical_dataset_bundle_descriptor.py")
REQUIRED_FIELDS = [
    "bundle_id",
    "bundle_name",
    "bundle_description",
...
class HistoricalDatasetBundleDescriptorContractTest(unittest.TestCase):
    """Protege o DTO oficial de descritor de bundle historico."""

    def test_contrato_importa_e_cria_com_valores_validos(self) -> None:
        descriptor = self._descriptor(packages=(self._package(),), total_packages=1)

        self.assertIsInstance(descriptor, HistoricalDatasetBundleDescriptor)
        self.assertEqual(descriptor.bundle_id, "historical_bundle_001")
        self.assertEqual(descriptor.packages[0].package_id, "historical_package_001")

    def test_campos_obrigatorios_estao_definidos(self) -> None:
        contract_fields = [
            field.name for field in fields(HistoricalDatasetBundleDescriptor)
        ]
        ...
        for field_name in REQUIRED_FIELDS:
            with self.subTest(field=field_name):
                ...
                with self.assertRaises(TypeError):
                    HistoricalDatasetBundleDescriptor(**partial_payload)  # type: ignore[arg-type]

    def test_type_hints_estao_presentes(self) -> None:
        hints = get_type_hints(HistoricalDatasetBundleDescriptor)
        expected_types = {
            "bundle_id": str,
            "bundle_name": str,
    ...
    def test_packages_deve_ser_colecao_tipada(self) -> None:
        ...
        self.assertEqual(self._descriptor(packages=(package,)).packages, (package,))
        with self.assertRaisesRegex(TypeError, "tuple"):
            self._descriptor(packages=[package])  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "HistoricalDatasetPackage"):
            self._descriptor(packages=({"package_id": "WDO"},))  # type: ignore[arg-type]

    def test_totais_devem_ser_inteiros_nao_negativos(self) -> None:
        self.assertEqual(self._descriptor(total_packages=0).total_packages, 0)
        self.assertEqual(self._descriptor(total_datasets=0).total_datasets, 0)
        with self.assertRaisesRegex(ValueError, "total_packages"):
            self._descriptor(total_packages=-1)
        with self.assertRaisesRegex(ValueError, "total_datasets"):
            self._descriptor(total_datasets=-1)
        with self.assertRaisesRegex(TypeError, "total_packages"):
            self._descriptor(total_packages="0")  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "total_datasets"):
            self._descriptor(total_datasets="0")  # type: ignore[arg-type]
    ...
    def test_nao_usa_dicionarios_livres(self) -> None:
        ...
        self.assertFalse(
            any(field.type is dict for field in fields(HistoricalDatasetBundleDescriptor))
        )
        self.assertFalse(any(isinstance(package, dict) for package in descriptor.packages))
    ...
    def test_contrato_e_dataclass_imutavel(self) -> None:
        ...
        with self.assertRaises(FrozenInstanceError):
            descriptor.bundle_id = "outro"  # type: ignore[misc]
    ...
    def test_camadas_consumidoras_permanecem_desacopladas(self) -> None:
        ...
        for path in paths:
            with self.subTest(path=str(path)):
                ...
                self.assertNotIn(
                    "historical.contracts.historical_dataset_bundle_descriptor",
                    source,
                )

    def test_nenhum_importer_exporter_provider_bundle_manager_descriptor_builder_ou_registry_concreto_foi_criado(
        self,
    ) -> None:
        historical_files = [
            path.name
        ...
        forbidden_names = (
            ...
            "historical_dataset_bundle_descriptor_builder.py",
            "historical_dataset_bundle_descriptor_factory.py",
            "historical_dataset_bundle_descriptor_service.py",
        )
    ...
    def _descriptor(self, **overrides: object) -> HistoricalDatasetBundleDescriptor:
        payload = self._descriptor_payload()
        payload.update(overrides)
        return HistoricalDatasetBundleDescriptor(**payload)  # type: ignore[arg-type]

    def _descriptor_payload(self) -> dict[str, object]:
        return {
            "bundle_id": "historical_bundle_001",
            "bundle_name": "Bundle historico WDO",
            "bundle_description": "Descritor administrativo de bundle WDO.",
            "packages": (self._package(),),
            "total_packages": 1,
            "total_datasets": 1,
            "bundle_version": "1.0",
            "created_at": "2026-06-27T20:15:00-03:00",
