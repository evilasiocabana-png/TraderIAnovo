"""Contrato historico de registry de bundles de datasets."""
...
from historical.contracts.historical_dataset_bundle_registry import (
    HistoricalDatasetBundleRegistry,
)
...
CONTRACT_PATH = Path("historical/contracts/historical_dataset_bundle_registry.py")
REQUIRED_FIELDS = [
    "registry_id",
    "registry_name",
    "bundles",
...
class HistoricalDatasetBundleRegistryContractTest(unittest.TestCase):
    """Protege o DTO oficial de registry de bundles historicos."""

    def test_contrato_importa_e_cria_com_valores_validos(self) -> None:
        registry = self._registry(bundles=(self._bundle(),), total_bundles=1)

        self.assertIsInstance(registry, HistoricalDatasetBundleRegistry)
        self.assertEqual(registry.registry_id, "bundle_registry_001")
        self.assertEqual(registry.bundles[0].bundle_id, "historical_bundle_001")

    def test_estado_vazio_e_permitido_quando_explicitamente_definido(self) -> None:
        registry = self._registry(bundles=(), total_bundles=0)

        self.assertEqual(registry.bundles, ())
        self.assertEqual(registry.total_bundles, 0)

    def test_campos_obrigatorios_estao_definidos(self) -> None:
        contract_fields = [
            field.name for field in fields(HistoricalDatasetBundleRegistry)
        ]
        ...
        for field_name in REQUIRED_FIELDS:
            with self.subTest(field=field_name):
                ...
                with self.assertRaises(TypeError):
                    HistoricalDatasetBundleRegistry(**partial_payload)  # type: ignore[arg-type]

    def test_type_hints_estao_presentes(self) -> None:
        hints = get_type_hints(HistoricalDatasetBundleRegistry)
        expected_types = {
            "registry_id": str,
            "registry_name": str,
    ...
    def test_bundles_deve_ser_colecao_tipada(self) -> None:
        ...
        self.assertEqual(self._registry(bundles=(bundle,)).bundles, (bundle,))
        with self.assertRaisesRegex(TypeError, "tuple"):
            self._registry(bundles=[bundle])  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "HistoricalDatasetBundle"):
            self._registry(bundles=({"bundle_id": "WDO"},))  # type: ignore[arg-type]

    def test_total_bundles_deve_ser_inteiro_nao_negativo(self) -> None:
        self.assertEqual(self._registry(total_bundles=0).total_bundles, 0)
        with self.assertRaisesRegex(ValueError, "non-negative"):
            self._registry(total_bundles=-1)
        with self.assertRaisesRegex(TypeError, "int"):
            self._registry(total_bundles="0")  # type: ignore[arg-type]
    ...
    def test_nao_usa_dicionarios_livres(self) -> None:
        ...
        self.assertFalse(
            any(field.type is dict for field in fields(HistoricalDatasetBundleRegistry))
        )
        self.assertFalse(any(isinstance(bundle, dict) for bundle in registry.bundles))
    ...
    def test_camadas_consumidoras_permanecem_desacopladas(self) -> None:
        ...
        for path in paths:
            with self.subTest(path=str(path)):
                ...
                self.assertNotIn(
                    "historical.contracts.historical_dataset_bundle_registry",
                    source,
                )

    def test_nenhum_importer_exporter_provider_bundle_manager_registry_ou_builder_foi_criado(
        self,
    ) -> None:
        historical_files = [
            path.name
        ...
        forbidden_names = (
            ...
            "historical_dataset_bundle_registry_builder.py",
            "historical_dataset_bundle_registry_factory.py",
            "historical_dataset_bundle_registry_service.py",
        )
    ...
    def _registry(self, **overrides: object) -> HistoricalDatasetBundleRegistry:
        payload = self._registry_payload()
        payload.update(overrides)
        return HistoricalDatasetBundleRegistry(**payload)  # type: ignore[arg-type]

    def _registry_payload(self) -> dict[str, object]:
        return {
            "registry_id": "bundle_registry_001",
            "registry_name": "Registry historico de bundles",
            "bundles": (self._bundle(),),
            "total_bundles": 1,
            "registry_version": "1.0",
            "generated_at": "2026-06-27T20:00:00-03:00",
        }
