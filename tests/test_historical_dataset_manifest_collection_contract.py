"""Contrato historico de colecao de manifests."""
...
from historical.contracts.historical_dataset_manifest_collection import (
    HistoricalDatasetManifestCollection,
)
...
CONTRACT_PATH = Path("historical/contracts/historical_dataset_manifest_collection.py")
REQUIRED_FIELDS = [
    "collection_id",
    "collection_name",
    "manifests",
...
class HistoricalDatasetManifestCollectionContractTest(unittest.TestCase):
    """Protege o DTO oficial de colecao de manifests historicos."""

    def test_contrato_importa_e_cria_com_valores_validos(self) -> None:
        collection = self._collection(manifests=(self._manifest(),), total_manifests=1)

        self.assertIsInstance(collection, HistoricalDatasetManifestCollection)
        self.assertEqual(collection.collection_id, "manifest_collection_001")
        self.assertEqual(collection.manifests[0].identity.dataset_id, "WDO_1m_2025")

    def test_estado_vazio_e_permitido_quando_explicitamente_definido(self) -> None:
        collection = self._collection(manifests=(), total_manifests=0)

        self.assertEqual(collection.manifests, ())
        self.assertEqual(collection.total_manifests, 0)

    def test_campos_obrigatorios_estao_definidos(self) -> None:
        contract_fields = [
            field.name for field in fields(HistoricalDatasetManifestCollection)
        ]
        ...
        for field_name in REQUIRED_FIELDS:
            with self.subTest(field=field_name):
                ...
                with self.assertRaises(TypeError):
                    HistoricalDatasetManifestCollection(**partial_payload)  # type: ignore[arg-type]

    def test_type_hints_estao_presentes(self) -> None:
        hints = get_type_hints(HistoricalDatasetManifestCollection)
        expected_types = {
            "collection_id": str,
            "collection_name": str,
    ...
    def test_manifests_deve_ser_colecao_tipada(self) -> None:
        ...
        self.assertEqual(self._collection(manifests=(manifest,)).manifests, (manifest,))
        with self.assertRaisesRegex(TypeError, "tuple"):
            self._collection(manifests=[manifest])  # type: ignore[arg-type]
        with self.assertRaisesRegex(TypeError, "HistoricalDatasetManifest"):
            self._collection(manifests=({"dataset_id": "WDO"},))  # type: ignore[arg-type]

    def test_total_manifests_deve_ser_inteiro_nao_negativo(self) -> None:
        self.assertEqual(self._collection(total_manifests=0).total_manifests, 0)
        with self.assertRaisesRegex(ValueError, "non-negative"):
            self._collection(total_manifests=-1)
        with self.assertRaisesRegex(TypeError, "int"):
            self._collection(total_manifests="0")  # type: ignore[arg-type]
    ...
    def test_nao_usa_dicionarios_livres(self) -> None:
        ...
        self.assertFalse(
            any(
                field.type is dict
                for field in fields(HistoricalDatasetManifestCollection)
            )
        )
        self.assertFalse(any(isinstance(manifest, dict) for manifest in collection.manifests))
    ...
    def test_camadas_consumidoras_permanecem_desacopladas(self) -> None:
        ...
        for path in paths:
            with self.subTest(path=str(path)):
                ...
                self.assertNotIn(
                    "historical.contracts.historical_dataset_manifest_collection",
                    source,
                )

    def test_nenhum_importer_provider_validator_builder_foi_criado(self) -> None:
        historical_files = [
            path.name
        ...
        forbidden_names = (
            "historical_dataset_manifest_collection_builder.py",
            "dataset_manifest_collection_builder.py",
    ...
    def _collection(self, **overrides: object) -> HistoricalDatasetManifestCollection:
        payload = self._collection_payload()
        payload.update(overrides)
        return HistoricalDatasetManifestCollection(**payload)  # type: ignore[arg-type]

    def _collection_payload(self) -> dict[str, object]:
        return {
            "collection_id": "manifest_collection_001",
            "collection_name": "Colecao historica de manifests",
            "manifests": (self._manifest(),),
            "total_manifests": 1,
            "created_at": "2026-06-27T18:15:00-03:00",
