"""Testes do acumulador financeiro M29 sem acesso ao MT5 real."""
...
from application.model29_basket_accumulator import (
    MODEL_29_FULL_EXIT_USD,
    Model29BasketManager,
    model29_entry_gate,
    model29_order_comment,
    model29_variant_id,
)
...
def _position(
    ...
    *,
    ...
    return SimpleNamespace(
        ...
        commission=0.0,
        fee=0.0,
        comment=f"TraderIA M29 S{source}",
    )


class Model29BasketAccumulatorTest(unittest.TestCase):
    def _manager(
        self,
        directory: str,
        positions: list[object],
    ) -> tuple[Model29BasketManager, _FakeExecutionService]:
        service = _FakeExecutionService(positions)
        manager = Model29BasketManager(
    ...
    def test_identidade_preserva_modelo_fonte(self) -> None:
        variant = model29_variant_id("MODELO_7_LAB_XAU_BTC")

        self.assertTrue(variant.endswith("SOURCE_M7"))
        self.assertEqual(model29_order_comment(variant), "TraderIA M29 S7")
        self.assertEqual(model29_order_comment("MODELO_29_BASKET_ACCUMULATOR_SOURCE_M10"), "TraderIA M29 S10")

    def test_identidade_recusa_m21_e_m22(self) -> None:
        for model in ("MODELO_21_XAU_M5_SMA_RSI_SMA50_SLOPE_REENTRY_TP75",
                      "MODELO_22_XAU_M5_SMA_RSI_TREND_FILTERS_REENTRY_TP75"):
            with self.assertRaises(ValueError):
                model29_variant_id(model)
    ...
    def test_full_exit_em_1000_fecha_todos_a_mercado_por_ticket(self) -> None:
        self.assertEqual(MODEL_29_FULL_EXIT_USD, 1000.0)
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(snapshot.exit_reason, "M29_FULL_EXIT_PLUS_1000_USD")
            self.assertEqual([call["ticket"] for call in service.close_calls], [22, 11])
            self.assertTrue(
                all(
                    call["reason"] == "M29_FULL_EXIT_PLUS_1000_USD"
    ...
    def test_estado_antigo_de_stop_ou_trailing_nao_fecha_a_cesta(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manager.state_path.write_text(
                ...
                '"exit_reason":"M29_GLOBAL_STOP_MINUS_300_USD",'
    ...
    def test_zeragem_pendente_aguarda_confirmacao_sem_repetir_ticket(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(second.exit_reason, "M29_FULL_EXIT_PLUS_1000_USD")
    ...
    def test_nova_rodada_exige_candle_posterior_a_zeragem(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            old_allowed, old_reason = model29_entry_gate(
                "2026-08-12T10:00:00+00:00",
                manager.state_path,
            )
            new_allowed, _ = model29_entry_gate(
            ...
            self.assertEqual(old_reason, "M29_SINAL_ANTIGO_DA_RODADA_ANTERIOR")
    ...
    def test_zeragem_manual_tambem_exige_candle_novo(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            cleared = manager.evaluate_once()
            allowed, reason = model29_entry_gate(
            ...
            self.assertEqual(reason, "M29_SINAL_ANTIGO_DA_RODADA_ANTERIOR")
    ...
    def test_gate_aceita_timestamp_do_dashboard_em_horario_de_brasilia(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            new_allowed, new_reason = model29_entry_gate(
                "26/08/2026 01:00",
                state_path,
            )
            ...
            self.assertEqual(old_reason, "M29_SINAL_ANTIGO_DA_RODADA_ANTERIOR")
            self.assertTrue(new_allowed)
            self.assertEqual(new_reason, "M29_NOVO_SINAL_LIBERADO")
