"""Modelo 29: acumulador de sinais com gestao financeira global da cesta."""
...
MODEL_29_ID = "MODELO_29_BASKET_ACCUMULATOR"
MODEL_29_ALPHA_ID = "ALPHA029_ACTIVE_MODEL_ACCUMULATOR"
MODEL_29_ALPHA_VERSION = "M29_ENTRY_V1"
MODEL_29_BETA_ID = "BETA029_FULL_EXIT_1000"
MODEL_29_BETA_VERSION = "M29_EXIT_V2"
MODEL_29_ENTRY_SOURCE = "MODEL_29_ACTIVE_SOURCE_SIGNAL"
MODEL_29_EXIT_POLICY = "M29_FULL_EXIT_1000_ONLY"
MODEL_29_FULL_EXIT_USD = 1000.0
MODEL_29_CLOSE_CONFIRMATION_SECONDS = 15.0
MODEL_29_STATE_PATH = Path(".traderia") / "model29_basket_state.json"
MODEL_29_SOURCE_NUMBERS = (7,)

_MODEL29_LOCK = threading.Lock()


class Model29ExecutionPort(Protocol):
...
def is_model29(value: object) -> bool:
    return str(value or "").upper().startswith(MODEL_29_ID)


def model29_source_model_id(value: object) -> str:
    normalized = str(value or "").upper()
    match = re.search(r"_SOURCE_M(\d{1,2})(?:_|\b|$)", normalized)
    return f"M{int(match.group(1))}" if match is not None else "N/D"


def model29_variant_id(source_operational_model: object) -> str:
    number = operational_model_number(source_operational_model)
    if number not in MODEL_29_SOURCE_NUMBERS:
        raise ValueError("M29 exige uma fonte operacional autorizada.")
    return f"{MODEL_29_ID}_SOURCE_M{number}"


def model29_order_comment(value: object) -> str:
    source = model29_source_model_id(value)
    return "TraderIA M29" if source == "N/D" else f"TraderIA M29 S{source[1:]}"


def model29_position_matches(position: object) -> bool:
    comment = str(getattr(position, "comment", "") or "").upper()
    return bool(re.search(r"\bM29\b", comment))


def model29_position_source(position: object) -> str:
    comment = str(getattr(position, "comment", "") or "").upper()
    match = re.search(r"\bS(\d{1,2})\b", comment)
    return f"M{int(match.group(1))}" if match is not None else "N/D"


def model29_entry_type(
    parameters: dict[str, Any] | None = None,
    *,
    entry_setup: object = "",
    alpha_id: object = "",
) -> str:
    """Resolve o tipo operacional real herdado pela copia M29."""
    payload = dict(parameters or {})
    candidates = (
        payload.get("m29_entry_type"),
    ...
    if setup:
        ...
        for part in reversed(parts):
            if part not in {"N/D", "NONE", "WAIT"} and not part.startswith("M29 <-"):
...
def model29_entry_type_token(entry_type: object) -> str:
...
def model29_position_entry_type_token(position: object) -> str:
    comment = str(getattr(position, "comment", "") or "").upper()
    match = re.search(r"\bT[0-9A-F]{8}\b", comment)
    return match.group(0) if match is not None else ""


def model29_position_net(position: object) -> float:
...
def model29_entry_gate(
    candle_time: object,
    state_path: Path = MODEL_29_STATE_PATH,
) -> tuple[bool, str]:
    """Permite reentrada nova e bloqueia apenas zeragem ou sinal antigo."""
    try:
        payload = json.loads(state_path.read_text(encoding="utf-8"))
    except (OSError, ValueError, TypeError):
        return True, "M29_SEM_RODADA_ANTERIOR"
    status = str(payload.get("status", "") or "").upper()
    if status in {"CLOSING", "EXIT_SUBMITTED", "EXIT_PARTIAL"}:
        return False, f"M29_AGUARDA_ZERAGEM_COMPLETA_{status}"
    accept_after = _parse_utc(payload.get("accept_signals_after"))
    if accept_after is None:
        return True, "M29_RODADA_LIBERADA"
    signal_time = _parse_utc(candle_time)
    if signal_time is None:
        return False, "M29_AGUARDA_NOVO_CANDLE_APOS_ZERAGEM"
    if signal_time <= accept_after:
        return False, "M29_SINAL_ANTIGO_DA_RODADA_ANTERIOR"
    return True, "M29_NOVO_SINAL_LIBERADO"
...
def _parse_utc(value: object) -> datetime | None:
    ...
    try:
        ...
        try:
            ...
            # time. M29 must accept that same contract when checking a new round.
...
@dataclass
class Model29BasketManager:
    """Fecha a cesta M29 somente no alvo financeiro global de US$1.000."""

    execution_service: Model29ExecutionPort
    state_path: Path = field(
        default_factory=lambda: MODEL_29_STATE_PATH
    )
    audit_path: Path = field(
        default_factory=lambda: Path(".traderia") / "model29_basket_audit.jsonl"
    )
    full_exit_usd: float = MODEL_29_FULL_EXIT_USD
    close_confirmation_seconds: float = MODEL_29_CLOSE_CONFIRMATION_SECONDS

    def evaluate_once(self) -> Model29BasketSnapshot:
        """Avalia e, quando necessario, encerra todos os tickets M29 Demo."""
        with _MODEL29_LOCK:
            positions = sorted(
                (
                    position
                    for position in self.execution_service.list_open_positions()
                    if model29_position_matches(position)
                ),
                key=model29_position_net,
                reverse=True,
            )
            ...
            if not positions:
                ...
                if state.status in {"CLOSING", "EXIT_SUBMITTED", "EXIT_PARTIAL"}:
                    ...
                    accept_after = now
                snapshot = Model29BasketSnapshot(
            ...
            net_result = round(sum(model29_position_net(item) for item in positions), 2)
            ...
            pending_exit = (
                ...
                and state.exit_reason == "M29_FULL_EXIT_PLUS_1000_USD"
            )
                ...
                results.append(
                    {
                        "ticket": int(getattr(position, "ticket", 0) or 0),
                        "symbol": str(getattr(position, "symbol", "") or "").upper(),
                        "source_model": model29_position_source(position),
                        "profit_before_close": model29_position_net(position),
    ...
    def _exit_reason(self, net_result: float) -> str:
        if net_result >= self.full_exit_usd:
            return "M29_FULL_EXIT_PLUS_1000_USD"
        return ""

    def _awaiting_close_confirmation(
        self,
        state: Model29BasketSnapshot,
        now: str,
    ) -> bool:
    ...
    def _load_state(self) -> Model29BasketSnapshot:
        try:
            payload = json.loads(self.state_path.read_text(encoding="utf-8"))
            allowed = Model29BasketSnapshot.__dataclass_fields__
            return Model29BasketSnapshot(
                **{key: value for key, value in payload.items() if key in allowed}
            )
        except (OSError, ValueError, TypeError):
            return Model29BasketSnapshot()

    def _write_state(self, snapshot: Model29BasketSnapshot) -> None:
    ...
    def _new_round_id() -> str:
        return "M29-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S") + "-" + uuid4().hex[:8]
