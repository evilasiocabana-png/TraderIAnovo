"""Experimento controlado para a Alpha002."""
...
class Alpha002ExperimentResult:
    """Resultado estrutural da execucao experimental da Alpha002."""
...
@dataclass
class Alpha002Experiment:
    """Executa replay da Alpha002 sem calcular metricas financeiras."""

    config: Alpha002Config
    candles: Sequence[Candle]
    strategy: Alpha002Strategy | None = None
    ...
    def run(self) -> Alpha002ExperimentResult:
        ...
        while replay.is_running:
            ...
            if candle is None:
                break
            signals.append(self._strategy().generate_signal(
        ...
        return Alpha002ExperimentResult(
    ...
    def _strategy(self) -> Alpha002Strategy:
        if self.strategy is not None:
            return self.strategy
        return Alpha002Strategy(config=self.config)
    ...
    def _feature_report(self, candle: Candle) -> FeatureReport:
        ...
        return FeatureReport(
            ...
            calculated_values={
                ...
                "vwap": float(candle.abertura),
                "vwap_distance_percent": self._vwap_distance(candle),
    ...
    def _vwap_distance(self, candle: Candle) -> float:
        if candle.abertura == 0:
            return 0.0
        return ((float(candle.fechamento) - float(candle.abertura))
                / float(candle.abertura)) * 100
