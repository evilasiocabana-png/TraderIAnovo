"""Experimento controlado para a Alpha201."""
...
class Alpha201ExperimentResult:
    """Resultado estrutural da execucao experimental da Alpha201."""
...
@dataclass
class Alpha201Experiment:
    """Executa replay da Alpha201 sem calcular metricas financeiras."""

    config: Alpha201Config
    candles: Sequence[Candle]
    strategy: Alpha201Strategy | None = None
    ...
    def run(self) -> Alpha201ExperimentResult:
        ...
        return Alpha201ExperimentResult(
    ...
    def _strategy(self) -> Alpha201Strategy:
        if self.strategy is not None:
            return self.strategy
        return Alpha201Strategy(config=self.config)
    ...
    def _feature_report(self, candle: Candle) -> FeatureReport:
        ...
        return FeatureReport(
            ...
            calculated_values={
                ...
                "structural_trend": self._structural_trend(candle),
                "directional_persistence": self._directional_persistence(candle),
    ...
    def _structural_trend(self, candle: Candle) -> str:
        if candle.fechamento >= candle.abertura:
            return "UP"
        return "DOWN"

    def _directional_persistence(self, candle: Candle) -> float:
        momentum = float(candle.fechamento) - float(candle.abertura)
        if momentum > 0:
            return 1.0
        if momentum < 0:
