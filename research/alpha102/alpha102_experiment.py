"""Experimento controlado para a Alpha102."""
...
class Alpha102ExperimentResult:
    """Resultado estrutural da execucao experimental da Alpha102."""
...
@dataclass
class Alpha102Experiment:
    """Executa replay da Alpha102 sem calcular metricas financeiras."""

    config: Alpha102Config
    candles: Sequence[Candle]
    strategy: Alpha102Strategy | None = None
    ...
    def run(self) -> Alpha102ExperimentResult:
        ...
        return Alpha102ExperimentResult(
    ...
    def _strategy(self) -> Alpha102Strategy:
        if self.strategy is not None:
            return self.strategy
        return Alpha102Strategy(config=self.config)
    ...
    def _feature_report(self, candle: Candle) -> FeatureReport:
        ...
        return FeatureReport(
            ...
            calculated_values={
                ...
                "trend_strength": self.config.minimum_trend_strength,
                "pullback_depth": self.config.minimum_pullback_depth,
                "structure_intact": True,
                "recovery_confirmation": float(candle.fechamento)
                - float(candle.abertura),
    ...
    def _trend_direction(self, candle: Candle) -> float:
        momentum = float(candle.fechamento) - float(candle.abertura)
        if momentum < 0:
            return -1.0
        return 1.0
