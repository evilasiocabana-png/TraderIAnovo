...
| FLOW-029 | M26 mostrou `SELL CONTINUATION` pronto e o preco rompeu a ordem Stop, mas nenhuma tentativa chegou ao provider | Duas instancias Streamlit escutavam simultaneamente a porta 8532 e dividiam o estado em memoria; no ciclo seguinte a decisao de continuidade desaparecia por ja estar rompida | Launcher, guardiao de RAM, snapshot compartilhado, M26, Robo Demo, provider e auditoria | Somente uma instancia pode ser dona da porta 8532; o guardiao encerra listeners adicionais. Uma continuidade confirmada permanece executavel por mais um ciclo e, se o preco ja rompeu o gatilho, o provider a converte em mercado sob o mesmo preflight atomico e sem duplicidade |
| FLOW-030 | M26 mostrava `BUY_STOP/SELL_STOP` pronto, mas a ordem nao aparecia no MT5 nem no log do provider | Depois de o adaptador canonico do M26 aprovar sua propria sequencia M5, o Robot Service reaplicava o regime legado e podia bloquear o candidato antes do provider | M26, adaptador canonico, Robot Service, regime legado, provider, MT5 e testes | M26, assim como M23-M25, e autoridade sobre seu proprio regime de entrada. O Robot Service nao reaplica o regime legado; selecao, temporalidade, Trade Plan, duplicidade, SL/TP e provider continuam obrigatorios. Um teste de regressao deve provar esse bypass antes de qualquer entrega |
| FLOW-031 | O M23 copiava sinais sem medir em qual contexto cada familia ajudava ou devolvia resultado | O historico possuia resultado e contrato-fonte, enquanto o Pattern Miner possuia o contexto causal M5, mas os dois artefatos nao eram ligados | Historico MT5, M23, Replay Pattern Miner, DashboardService, UI e testes | O Replay liga cada negocio M23 atual ao ultimo `EventRecord` fechado anterior a entrada. Cada fonte e direcao recebe sua propria divisao 60/20/20 e suas proprias familias causais. Somente regras com pelo menos 20 casos, tres casos por faixa e sinal igual em descoberta, validacao e OOS recebem `APPROVE/BLOCK`. O runtime le o JSON leve em `INDIVIDUAL_BLOCK_ONLY`: apenas `BLOCK` da propria fonte impede a nova entrada antes do provider; ausencia de evidencia preserva o sinal original |
| FLOW-032 | Uma regra agregada `ALL_SOURCES` podia usar o resultado de uma fonte para bloquear sinais de setups diferentes | A divisao temporal era feita na cesta inteira e o bloqueio global de RSI tinha precedencia sobre a evidencia individual | Replay M23, regra de fonte, DashboardService, provider e auditoria | Cada modelo e direcao possui cronologia independente; regras `ALL_SOURCES` deixam de ser geradas ou executadas. Um `BLOCK` validado prevalece somente sobre dimensoes favoraveis da mesma fonte, sem contaminar os demais modelos |
| FLOW-033 | O diagnostico alternava para `MT5 OFFLINE` apesar de o terminal responder normalmente | A leitura homogenea abria uma sonda externa por par, o Position Manager materializava 52 MB de JSONL a cada ciclo e os scripts ainda reiniciavam o Streamlit no limite antigo de 900 MB | Provider MT5, cache compartilhado, Position Manager, log de execucao, launcher, guardiao de RAM e diagnostico visual | A leitura homogenea usa um unico lote MT5; a gestao percorre o JSONL de tras para frente e para ao encontrar os tickets abertos; o snapshot primario online e publicado antes dos suplementos; launcher e guardiao obedecem ao limite arquitetural de 1600 MB |
| FLOW-034 | A aba Historico MT5 nao comprovava se os padroes M28 descobertos nos 100.000 candles continuavam validos depois do corte | Base de descoberta e dados novos nao possuiam fronteira temporal nem conferencia comum com o log de execucao | Replay M28, provider MT5 read-only, Historico MT5, log Demo e DashboardService | Os CSVs originais ficam congelados; um botao baixa somente M5 fechados posteriores ao corte para arquivos incrementais isolados e reconstrui os sinais com o mesmo motor causal, comparando padrao, lado, entrada, SL e TP com cada tentativa MT5 sem participar do ciclo leve |
| FLOW-035 | O grafico Teorico x MT5 mostrava somente tickets pareados e ocultava sinais sem execucao e execucoes sem pareamento | A mesma amostra era usada para facilitar a sobreposicao, eliminando justamente os casos necessarios para medir a conversao do sinal | Replay forward M28, Historico MT5, log Demo, auditoria e graficos | As curvas principais sao independentes e normalizadas em R: azul usa todos os sinais teoricos resolvidos e verde todas as operacoes M28 encerradas; o funil separado mostra sinais, tentativas, conferencias, rejeicoes e ausencias. Pareamento fica restrito a auditoria detalhada |
| FLOW-036 | Sinais teoricos de periodos em que o app, o MT5 ou o robo estavam indisponiveis apareciam como se o executor tivesse falhado | Os estados do robo e do ciclo eram sobrescritos em arquivos correntes, sem historico temporal suficiente para provar disponibilidade por candle e ativo | Ciclo leve, M28, Replay forward, Historico MT5 e auditoria | Um heartbeat leve registra somente evidencias operacionais. A comparacao aceita um sinal apenas quando robo armado, M28 selecionado, ciclo concluido e dados do ativo estavam validos no fechamento M5; periodos antigos ou lacunas sem heartbeat ficam fora da amostra e explicitamente nao observaveis |
