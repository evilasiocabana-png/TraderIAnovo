...
| FLOW-031 | O M23 copiava sinais sem medir em qual contexto cada familia ajudava ou devolvia resultado | O historico possuia resultado e contrato-fonte, enquanto o Pattern Miner possuia o contexto causal M5, mas os dois artefatos nao eram ligados | Historico MT5, M23, Replay Pattern Miner, DashboardService, UI e testes | O Replay liga cada negocio M23 atual ao ultimo `EventRecord` fechado anterior a entrada, separa 60/20/20 e pesquisa familias por fonte/direcao. Somente regras com pelo menos 20 casos, tres casos por faixa e sinal igual em descoberta, validacao e OOS recebem `APPROVE/BLOCK`. O runtime le o JSON leve em `ACTIVE_BLOCK_ONLY`: apenas `BLOCK` impede a nova entrada antes do provider; ausencia ou conflito de evidencia preserva o sinal original |
| FLOW-032 | O M23 devolvia parte relevante do patrimonio ao continuar vendendo depois de o RSI causal ficar abaixo de 30 | As perdas permaneceram negativas em descoberta, validacao e OOS, enquanto o mesmo corte em BUY nao foi estavel | Replay M23, regra de carteira, DashboardService, provider e auditoria | O minerador cria uma regra RSI por direcao para `ALL_SOURCES`. `SELL + RSI_LT30` recebe `BLOCK` e prevalece sobre aprovacao individual; BUY e demais zonas continuam inalterados |
