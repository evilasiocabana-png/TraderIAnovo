...
from core.weekly_robot_schedule import (
    WeeklyRobotScheduleDecision,
    weekly_robot_schedule_decision,
)
...
MT5_OPERATIONAL_MODEL_CHECKBOX_PREFIX = "mt5_operational_model_checkbox"
MT5_OPERATIONAL_MODEL_SYNC_KEY = "mt5_operational_model_last_synced"
MT5_OPERATIONAL_MODEL_FORM_KEY = "mt5_operational_model_form"
MT5_OPERATIONAL_MODEL_REVISION_KEY = "mt5_operational_model_revision"
...
MT5_HISTORICAL_LAB_OPERATIONAL_MODELS = (
    MT5_LAB_OPERATIONAL_MODELS | set(MT5_RETIRED_OPERATIONAL_MODEL_IDS)
)
...
MT5_WEEKLY_ROBOT_SCHEDULE_STATE_PATH = (
    Path(".traderia") / "weekly_robot_schedule_state.json"
)
...
MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_STARTED = False
...
MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_NAME = "TraderIA-Weekly-Robot-Schedule"
MT5_WEEKLY_ROBOT_SCHEDULE_INTERVAL_SECONDS = 10.0
MT5_WEEKLY_ROBOT_FLAT_CHECK_SECONDS = 60.0
    ...
    while True:
        if (
            ...
            try:
                ...
                decisions.update(
                    service.get_xau_m5_operational_decision_snapshot()
                )
                decisions.update(
                    service.get_forex_m5_sma_rsi_decision_snapshot()
                )
                publish_background_snapshot(
...
def _start_weekly_robot_schedule_once(force: bool = False) -> None:
    """Starts the weekly guard even when the robot is currently offline."""
    global MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_STARTED
    if is_background_runtime_running(MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_NAME):
        MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_STARTED = True
        return
    if not (force or _weekly_robot_schedule_enabled()):
        return
    start_background_runtime_once(
        MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_NAME,
        _weekly_robot_schedule_cycle,
    )
    MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_STARTED = is_background_runtime_running(
        MT5_WEEKLY_ROBOT_SCHEDULE_THREAD_NAME
    )


def _weekly_robot_schedule_cycle() -> None:
    service = DashboardService()
    while True:
        try:
            _enforce_weekly_robot_schedule(service)
        except Exception as exc:  # noqa: BLE001 - guard must survive MT5 failures
            _write_weekly_robot_schedule_state(
                weekly_robot_schedule_decision(),
                action="ERROR",
                close_result={"status": "ERROR", "message": str(exc)},
            )
        time.sleep(MT5_WEEKLY_ROBOT_SCHEDULE_INTERVAL_SECONDS)


def _enforce_weekly_robot_schedule(
    service: DashboardService,
    *,
    now: datetime | None = None,
) -> dict[str, object]:
    decision = weekly_robot_schedule_decision(now)
    online_state = _load_demo_robot_online_state()
    previous = _load_weekly_robot_schedule_state()
...
def _weekly_flat_check_due(
    previous: dict[str, object],
    decision: WeeklyRobotScheduleDecision,
) -> bool:
    if str(previous.get("schedule_status") or "") != decision.status:
        return True
    raw = str(previous.get("last_flat_check_at") or "")
    try:
        checked_at = datetime.fromisoformat(raw)
    except ValueError:
        return True
    elapsed = datetime.now(timezone.utc) - checked_at.astimezone(timezone.utc)
    close_result = dict(previous.get("close_result") or {})
    interval = (
        MT5_WEEKLY_ROBOT_SCHEDULE_INTERVAL_SECONDS
        if int(close_result.get("remaining", 0) or 0) > 0
        else MT5_WEEKLY_ROBOT_FLAT_CHECK_SECONDS
    )
    return elapsed.total_seconds() >= interval


def _write_weekly_robot_schedule_state(
    decision: WeeklyRobotScheduleDecision,
    *,
    action: str,
    close_result: dict[str, object],
    ...
    previous = _load_weekly_robot_schedule_state()
    payload = {
        "schedule_status": decision.status,
        "operating": decision.operating,
        "reason": decision.reason,
        "evaluated_at_brt": decision.evaluated_at_brt,
        "next_transition_brt": decision.next_transition_brt,
        "action": str(action),
        "close_result": dict(close_result),
        "last_flat_check_at": (
            datetime.now(timezone.utc).isoformat()
            if "FLAT_CHECK" in str(action) or str(action) == "ERROR"
            else previous.get("last_flat_check_at")
        ),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        MT5_WEEKLY_ROBOT_SCHEDULE_STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
        temporary = MT5_WEEKLY_ROBOT_SCHEDULE_STATE_PATH.with_suffix(".json.tmp")
        temporary.write_text(
            json.dumps(payload, ensure_ascii=True, indent=2),
            encoding="utf-8",
        )
        temporary.replace(MT5_WEEKLY_ROBOT_SCHEDULE_STATE_PATH)
    except OSError:
        return


def _load_weekly_robot_schedule_state() -> dict[str, object]:
    try:
        payload = json.loads(
            MT5_WEEKLY_ROBOT_SCHEDULE_STATE_PATH.read_text(encoding="utf-8")
        )
        ...
        if online and _mt5_forex_market_cycle_allowed_now():
            ...
            try:
                with MT5_FOREX_CYCLE_LOCK:
                    ...
                    previous_snapshot = get_background_snapshot(
                        MT5_FOREX_SHARED_SNAPSHOT_KEY
                    )
                    # Uma sonda transitoria ocupada nao pode apagar um snapshot
                    # online ja confirmado. O robo continua usando o ultimo
                    # conjunto valido ate a proxima leitura completa.
                    if (
                        _mt5_snapshot_has_received_candles(runtime_snapshot)
...
def _mt5_snapshot_has_received_candles(snapshot: object) -> bool:
    """Confirma que o snapshot possui ao menos um mercado MT5 utilizavel."""
    pairs = list(getattr(snapshot, "pairs", []) or [])
    return any(
...
def _apply_persisted_operational_model_to_service(service: DashboardService) -> None:
    ...
    if not model:
        return
    selections = _load_persisted_mt5_operational_selections()
    models = tuple(item for item in selections if item in MT5_ACTIVE_SOURCE_MODEL_IDS)
    basket_models = tuple(
        item
    ...
    if callable(multi_setter):
        multi_setter(
            ...
            direct_models_enabled=bool(models),
        )
        return
...
def _load_demo_robot_background_state() -> dict[str, object]:
    path = Path(".traderia") / "mt5_demo_robot_background_state.json"
    try:
...
def _load_mt5_trade_audit_report_locked(service: DashboardService) -> object | None:
    # A auditoria e secundaria em relacao ao ciclo de mercado/execucao. Em
    # especial com varias abas Streamlit abertas, esperar pelo lock aqui cria
    # uma fila de relatorios que pode impedir o robo de renovar os candles.
    # Se o ciclo MT5 estiver ocupado, preservamos o ultimo relatorio e tentamos
    # novamente no proximo refresh.
...
def ensure_mt5_forex_initial_load(service: DashboardService) -> None:
    ...
    if shared is not None:
        return
    # O ciclo de fundo e o unico dono da leitura MT5 na inicializacao. Com um
    # universo amplo, repetir a mesma sonda aqui bloquearia a primeira pintura.
    ...
    try:
        ...
        decisions = service.refresh_lab_operational_decision_snapshot()
        decisions.update(service.get_xau_m5_operational_decision_snapshot())
        decisions.update(service.get_forex_m5_sma_rsi_decision_snapshot())
        runtime_snapshot = service.get_mt5_forex_runtime_view_model()
...
def _maybe_run_mt5_forex_auto_cycle(
    ...
    # A interface nunca assume a leitura MT5. O ciclo em segundo plano publica
    # o snapshot e a tela apenas o consome; isso impede que uma reconexao web
    # bloqueie o Streamlit durante uma chamada nativa do terminal.
    return data, forex
...
def _maybe_refresh_mt5_trade_audit_report(
    ...
    *,
    ...
    cache_is_incomplete = (
        ...
        and not _mt5_trade_audit_report_has_closed_rows(cached_report)
    )
    if not force and cached_report is not None and not cache_is_incomplete:
...
def _mt5_trade_audit_report_has_closed_rows(report: object) -> bool:
    """Distingue o snapshot completo da leitura leve de posicoes abertas."""
...
def _mt5_operational_model_labels() -> dict[str, str]:
    labels = {
        ...
        MT5_OPERATIONAL_MODEL_3: "Modelo 3 - XAU M5 RSI50",
        MT5_OPERATIONAL_MODEL_4: "Modelo 4 - Lab contextual MTF",
        MT5_OPERATIONAL_MODEL_5: "Modelo 5 - consolidado M1-M4",
        MT5_OPERATIONAL_MODEL_6: "Modelo 6 - expansao Forex Lab",
        MT5_OPERATIONAL_MODEL_7: "Modelo 7 - Lab Ouro e Bitcoin",
        MT5_OPERATIONAL_MODEL_8: "Modelo 8 - XAUUSD M5 SMA20/50 + RSI14",
        MT5_OPERATIONAL_MODEL_9: "Modelo 9 - XAUUSD M5 + ADX",
        MT5_OPERATIONAL_MODEL_10: "Modelo 10 - Setup C: M8 + distancia/ATR",
        MT5_OPERATIONAL_MODEL_11: "Modelo 11 - XAUUSD M5 + inclinacao SMA50",
        MT5_OPERATIONAL_MODEL_12: "Modelo 12 - XAUUSD M5 filtros combinados",
        MT5_OPERATIONAL_MODEL_13: "Modelo 13 - Forex M5 reentrada",
        MT5_OPERATIONAL_MODEL_14: "Modelo 14 - Forex M5 + ADX",
        MT5_OPERATIONAL_MODEL_15: "Modelo 15 - Forex M5 distancia/ATR",
    ...
    for model_id in XAU_IMPROVED_REENTRY_MODEL_IDS:
        ...
        if spec is not None:
            labels[model_id] = (
                f"Modelo {spec.number} - M{spec.number - 10} melhorado, "
                "TP 7,5 so na reentrada"
            )
    labels[MT5_OPERATIONAL_MODEL_8_TO_22] = "Modelos M8-M22"
    labels[MT5_OPERATIONAL_MODEL_ALL] = "Todos os modelos ativos"
    return labels
...
def _persist_mt5_operational_model_checkbox_selection(model: str) -> None:
    """Compatibilidade: aplica a mesma multisselecao usada pelo formulario."""
    for candidate in (
...
def _persist_mt5_operational_model_form_selection() -> str:
    """Persiste exatamente as caixas marcadas, sem exclusividade entre modelos."""
    _mark_ui_critical_interaction()
    sources = MT5_ACTIVE_SOURCE_MODEL_IDS
    selectable = tuple(MT5_SELECTABLE_OPERATIONAL_MODEL_IDS)
    persisted_selections = set(_load_persisted_mt5_operational_selections())
    all_checked = bool(
        st.session_state.get(
            _mt5_operational_model_checkbox_key(MT5_OPERATIONAL_MODEL_ALL),
            False,
    ...
    all_was_selected = persisted_selections == set(selectable)
    if all_checked and not (all_was_selected and len(manual_selections) < len(selectable)):
        selections = selectable
        for candidate in selectable:
            st.session_state[_mt5_operational_model_checkbox_key(candidate)] = True
    else:
        selections = manual_selections
        if all_was_selected and len(manual_selections) < len(selectable):
            st.session_state[
                _mt5_operational_model_checkbox_key(MT5_OPERATIONAL_MODEL_ALL)
            ] = False
    if not selections:
        selections = (MT5_OPERATIONAL_MODEL_1,)
        st.session_state[
            _mt5_operational_model_checkbox_key(MT5_OPERATIONAL_MODEL_1)
        ] = True

    selected_models = tuple(model for model in selections if model in sources)
    selected_baskets = tuple(
        model
    ...
    _persist_mt5_operational_model(
        selected,
...
def _mt5_operational_model_state_revision() -> str:
    try:
        payload = json.loads(
            MT5_OPERATIONAL_MODEL_STATE_PATH.read_text(encoding="utf-8")
        )
    except (OSError, json.JSONDecodeError):
        return "SEM_ESTADO"
...
def _render_mt5_operational_model_selector() -> str:
    ...
    persisted_models = tuple(
        item for item in persisted_selections if item in MT5_ACTIVE_SOURCE_MODEL_IDS
    )
    ...
    for candidate in options:
        checked = candidate in persisted_selections
        if candidate == MT5_OPERATIONAL_MODEL_ALL:
            checked = set(persisted_selections) == set(MT5_SELECTABLE_OPERATIONAL_MODEL_IDS)
    ...
    st.session_state[MT5_OPERATIONAL_MODEL_REVISION_KEY] = persisted_revision
    ...
    with st.container(border=True):
        ...
        summary_label = (
            "TODOS"
            if set(persisted_selections) == set(MT5_SELECTABLE_OPERATIONAL_MODEL_IDS)
            else _mt5_operational_model_short_label(persisted_selections[0])
            if len(persisted_selections) == 1
            else f"{len(persisted_selections)} MODELOS"
        )
        ...
        with st.form(MT5_OPERATIONAL_MODEL_FORM_KEY, border=False):
            checkbox_columns = st.columns(6)
            for index, candidate in enumerate(options):
                model_number = operational_model_number(candidate)
                short_label = (
                    "Todos"
                    if candidate == MT5_OPERATIONAL_MODEL_ALL
                    else f"M{model_number}"
                )
                checkbox_columns[index % 6].checkbox(
                    short_label,
    ...
    if MT5_OPERATIONAL_MODEL_2 in persisted_selections:
        st.warning(
            "M2 ativo nos oito pares: entrada M15 com direcao H1, EMA9/21, "
            "ADX14 > 20, pullback entre as medias e candle fechado de "
            "confirmacao. SL inicial 1,25 ATR e TP fixo 2R."
        )
    if len(persisted_selections) > 1:
        st.warning(
            "Todos os modelos ativos marcados podem enviar ordem. "
            "O mesmo sinal pode gerar a ordem direta do modelo e outra ordem "
            "separada da cesta M23 quando ela estiver marcada."
        )
    if len(persisted_selections) > 1:
        selected_labels = ", ".join(
            _mt5_operational_model_short_label(model)
            for model in persisted_selections
        )
    ...
    if basket_mode:
        st.warning(
            ...
            "quando a cesta atingir +US$1.000 liquidos."
        )
        basket = _load_mt5_model23_basket_state()
    ...
    if selected == MT5_OPERATIONAL_MODEL_7:
        st.warning(
            "M7 ativo: usa o mesmo Lab e Trade Plan do M1 somente em XAUUSD "
            "e BTCUSD. BITCOIN do CSV foi mapeado para BTCUSD da Pepperstone."
        )
    dynamic_spec = dynamic_exit_model_spec(selected)
    if dynamic_spec is not None:
        st.warning(
            f"{dynamic_spec.short_name} ativo: copia integralmente entrada, SL "
            f"inicial e TP do {dynamic_spec.source_short_name}. Depois da abertura, "
            "o Position Manager pode manter ou melhorar o SL a partir de 1,50R; "
            "EARLY_EXIT e FULL_EXIT permanecem desligados."
        )
    if selected == MT5_OPERATIONAL_MODEL_8:
        st.warning(
            "M8 ativo somente em XAUUSD/M5: entra a mercado quando o RSI14 "
            "fica acima/abaixo de 50 na direcao de SMA20/SMA50; SL alem do ultimo pivo 2+2; "
            "sem TP; FULL EXIT no cruzamento fechado 70/30 ou se as medias inverterem."
        )
    if selected in XAU_TREND_FILTER_MODEL_IDS:
        spec = trend_filter_spec(selected)
        if spec is not None:
            st.warning(
                f"M{spec.number} / Setup {spec.setup} ativo somente em XAUUSD/M5. "
                "Mantem entrada RSI50, reentrada Stop, SL estrutural e Full Exit 70/30 do M8; "
                + (
                    "a entrada inicial nao usa TP, a unica reentrada usa TP de 7,5 pontos."
                    if selected in XAU_IMPROVED_REENTRY_MODEL_IDS
                    else "o filtro adicional de lateralidade precisa estar liberado."
                )
            )
    if selected in FOREX_SMA_RSI_MODEL_IDS:
        spec = forex_sma_rsi_spec(selected)
        if spec is not None:
            st.warning(
                f"M{spec.number} / Setup {spec.setup} ativo nos 17 pares Forex/M5. "
                "Entrada inicial a mercado, reentrada Stop, SL no pivo 2+2, "
                "sem TP e Full Exit por RSI 70/30 ou inversao SMA20/50."
            )
    selected_label = next(
        (
            label
...
def _load_persisted_mt5_operational_models() -> tuple[str, ...]:
    try:
        data = json.loads(MT5_OPERATIONAL_MODEL_STATE_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return (MT5_OPERATIONAL_MODEL_1,)
    ...
    if "selections" in data:
        return tuple(
            dict.fromkeys(
                normalized
                for item in list(data.get("selections") or [])
                for normalized in [
                    LEGACY_MT5_OPERATIONAL_MODELS.get(
                        str(item or "").upper(), str(item or "").upper()
                    )
                ]
                if normalized in MT5_ACTIVE_SOURCE_MODEL_IDS
            )
        )
    selected: list[str] = []
...
def _load_persisted_mt5_operational_selections() -> tuple[str, ...]:
    ...
    for item in list(data.get("selections") or []):
        ...
        if normalized in selectable and normalized not in selected:
            selected.append(normalized)
    if selected:
        return tuple(selected)

    # Migra estados antigos sem alterar a escolha que o usuario havia feito.
    mode = _valid_mt5_operational_model(data.get("model"))
    direct = list(_load_persisted_mt5_operational_models())
    if mode == MT5_OPERATIONAL_MODEL_ALL:
        return selectable
    if mode in {MT5_OPERATIONAL_MODEL_23, MT5_OPERATIONAL_MODEL_WITH_23}:
...
def _persist_mt5_operational_model(
    model: object,
    *,
    models: tuple[str, ...] | list[str] | None = None,
    selections: tuple[str, ...] | list[str] | None = None,
) -> None:
    normalized = _valid_mt5_operational_model(model)
    selected: list[str] = []
    for item in list(models or []):
        candidate = LEGACY_MT5_OPERATIONAL_MODELS.get(
            str(item or "").upper(),
            str(item or "").upper(),
        )
    ...
    persisted_selections: list[str] = []
    for item in list(selections or []):
        candidate = LEGACY_MT5_OPERATIONAL_MODELS.get(
            str(item or "").upper(), str(item or "").upper()
        )
        if (
            candidate in MT5_SELECTABLE_OPERATIONAL_MODEL_IDS
            and candidate not in persisted_selections
        ):
            persisted_selections.append(candidate)
    if selections is None:
        persisted_selections = list(selected)
        if normalized in {
            MT5_OPERATIONAL_MODEL_23,
            MT5_OPERATIONAL_MODEL_WITH_23,
        }:
...
def _mt5_operational_model_short_label(model: str) -> str:
    ...
    if normalized == MT5_OPERATIONAL_MODEL_CUSTOM:
        return "SELECIONADOS"
    ...
    if normalized == MT5_OPERATIONAL_MODEL_7:
        return "M7"
    if normalized == MT5_OPERATIONAL_MODEL_8:
        return "M8"
    if normalized == MT5_OPERATIONAL_MODEL_9:
        return "M9"
    if normalized == MT5_OPERATIONAL_MODEL_10:
        return "M10"
    if normalized == MT5_OPERATIONAL_MODEL_11:
        return "M11"
    if normalized == MT5_OPERATIONAL_MODEL_12:
        return "M12"
    if normalized == MT5_OPERATIONAL_MODEL_13:
        return "M13"
    if normalized == MT5_OPERATIONAL_MODEL_14:
        return "M14"
    if normalized == MT5_OPERATIONAL_MODEL_15:
        return "M15"
    if normalized == MT5_OPERATIONAL_MODEL_16:
        return "M16"
    if normalized == MT5_OPERATIONAL_MODEL_17:
        return "M17"
    if normalized in XAU_IMPROVED_REENTRY_MODEL_IDS:
        spec = trend_filter_spec(normalized)
    ...
    if normalized == MT5_OPERATIONAL_MODEL_ALL:
        return "TODOS"
    if normalized == MT5_OPERATIONAL_MODEL_8_TO_17:
        return "M8-M22"
    return "MODELO 1"
...
def _mt5_operational_model_enabled(selected: str, model: str) -> bool:
    ...
    if normalized == MT5_OPERATIONAL_MODEL_CUSTOM:
        return str(model or "").upper() in _load_persisted_mt5_operational_selections()
    return (
        normalized == MT5_OPERATIONAL_MODEL_ALL
        or normalized == model
        or (
            normalized in {
                MT5_OPERATIONAL_MODEL_CUSTOM,
...
def exibir_relatorios_dashboard(service: DashboardService, data: object) -> None:
    ...
    audit_rows = list(getattr(report, "rows", []) or [])
    rows = [row for row in audit_rows if _is_mt5_actual_operation(row)]
    ignored_audit_rows = len(audit_rows) - len(rows)
    st.info(_mt5_position_manager_status_message(rows))
    if ignored_audit_rows:
        st.caption(
            f"{ignored_audit_rows} registro(s) apenas tecnico(s), pendente(s) ou "
            "nao encontrado(s) no MT5 foram excluidos das negociacoes, dos "
            "resultados e dos graficos."
        )
    if not rows:
        st.info("Nenhuma negociacao confirmada por ticket no MT5 foi encontrada.")
        return
    ...
    st.info(
        "Em negociacao mostra POSITION e ORDER_OPEN confirmadas por ticket. "
        "Historico mostra somente operacoes fechadas encontradas no MT5."
    )
    st.info(
...
def _is_mt5_actual_operation(row: object) -> bool:
    """Aceita somente uma posicao, ordem ou negocio confirmado pela fonte MT5."""
    operation_status = str(getattr(row, "operation_status", "") or "").upper()
    if operation_status not in {"ABERTA", "ORDEM_ABERTA", "FECHADA/HISTORICO"}:
        return False
    if not bool(getattr(row, "mt5_found", False)):
        return False
    ticket = getattr(row, "mt5_ticket", None)
    if ticket in {None, "", 0, "0", "N/D"}:
        return False
    return _mt5_trade_side_label(row) in {"BUY", "SELL"}


def _mt5_trade_side_label(row: object) -> str:
    """Prefere o lado MT5, sem deixar o marcador N/D esconder o lado local."""
    mt5_side = str(getattr(row, "mt5_side", "") or "").upper()
    if mt5_side in {"BUY", "SELL"}:
        return mt5_side
    local_side = str(getattr(row, "side", "") or "").upper()
    return local_side if local_side in {"BUY", "SELL"} else "N/D"
...
def _is_model23_trade_row(row: object) -> bool:
    model = str(getattr(row, "operational_model", "") or "").upper()
    comment = str(getattr(row, "mt5_comment", "") or "").upper()
    return model.startswith(MT5_OPERATIONAL_MODEL_23) or " M23" in comment


def _mt5_open_profit_summary(rows: list[object]) -> dict[str, str]:
    projected_rows = [row for row in rows if not _is_model23_trade_row(row)]
    projected_profit = sum(
        float(getattr(row, "projected_profit", 0.0) or 0.0)
        for row in projected_rows
    )
    projected_loss = sum(
        float(getattr(row, "projected_loss", 0.0) or 0.0)
        for row in projected_rows
    )
...
def _mt5_open_trade_compact_row(row: object) -> dict[str, object]:
    ...
    return {
        ...
        "Lado": _mt5_trade_side_label(row),
        "Resultado MT5": _mt5_open_trade_money(row, "mt5_realized_profit"),
        "Lucro projetado": (
            "CESTA GLOBAL" if _is_model23_trade_row(row)
            else _mt5_open_trade_money(row, "projected_profit")
        ),
        "Risco atual": (
            "SL INDIVIDUAL DA FONTE" if _is_model23_trade_row(row)
            else _mt5_open_trade_money(row, "projected_loss")
        ),
...
def _mt5_trade_audit_compact_row(row: object) -> dict[str, object]:
    ...
    return {
        ...
        "Pico lucro aberto": _optional_number(
...
def _exibir_evolucao_patrimonial_mt5(report: object, rows: list[object]) -> None:
    ...
    visible_individual_models = [
        f"MODELO {index}" for index in MT5_ACTIVE_REPORT_MODEL_NUMBERS
    ]
    model_rows = {
...
def _mt5_equity_main_chart_model_selection() -> str:
    ...
    all_selected = all_column.checkbox(
        ...
        help="Marca todos os modelos operacionais ativos no grafico principal.",
    )
    selected_models: list[str] = []
    for start in range(0, len(MT5_ACTIVE_REPORT_MODEL_NUMBERS), 5):
        columns = st.columns(5)
        for column, index in zip(
            columns,
            MT5_ACTIVE_REPORT_MODEL_NUMBERS[start : start + 5],
        ):
            model = f"M{index}"
...
def _mt5_rows_for_equity_model_selection(
    ...
    if normalized == "TODOS":
        return [
            ...
            for row in rows
            if any(
                _mt5_equity_row_matches_active_model(row, model_key)
    ...
    model_keys = {
        ...
        for index in MT5_ACTIVE_REPORT_MODEL_NUMBERS
        for alias in (f"M{index}", f"MODELO{index}")
    }
    targets = {model_keys[key] for key in selected_keys if key in model_keys}
    if not targets:
        if selected_keys:
            return []
        targets = active_keys
...
def _render_mt5_equity_chart(
    ...
    *,
    ...
    with st.container(
        ...
        st.markdown(f"##### {title}")
        setup_summary = _mt5_equity_model_setup_summary(model_filter)
        if setup_summary:
            st.caption(setup_summary)
...
def _mt5_equity_model_setup_summary(model_filter: str) -> str:
    normalized = str(model_filter or "").upper().replace("_", " ").strip()
    summaries = {
        "MODELO 1": (
            "Lab vencedor por par e timeframe | entrada, SL e TP fixos do plano"
        ),
        "MODELO 2": (
            "Trend Pullback H1 -> M15 | ADX > 20 | SL 1,25 ATR | alvo 2R"
        ),
        "MODELO 3": (
            "XAUUSD/M5 | BUY RSI14>50 + fechamento>SMA20 | SELL RSI14<50 + "
            "fechamento<SMA20 | janela deslizante 200 velas fechadas | "
            "Full Exit pelo RSI oposto | "
            "SL no pivo 2+2 | sem TP fixo"
        ),
        "MODELO 4": (
            "Liquidity Reclaim M30 | EMA 34/144 + wick/RSI | alvo 3R"
        ),
        "MODELO 5": (
            "Melhor plano consolidado de M1-M4 por par | SL/TP fixos"
        ),
        "MODELO 6": (
            "Lab Forex expandido | vencedor individual dos nove novos pares "
            "| SL/TP fixos do plano"
        ),
        "MODELO 7": (
            "Lab Ouro e Bitcoin | vencedor individual de XAUUSD e BTCUSD "
            "| SL/TP fixos do plano"
        ),
        "MODELO 8": (
            "XAUUSD/M5 | entrada pelo nivel RSI14/50 com direcao SMA20/50 "
            "| Full Exit por cruzamento fechado RSI 70/30 ou inversao das medias | sem TP"
        ),
        "MODELO 9": (
            "Setup B XAUUSD/M5 | base M8 + ADX14 > 25"
        ),
        "MODELO 10": (
            "Setup C XAUUSD/M5 | base M8 + distancia SMA20/50 por ATR >= 0,25"
        ),
        "MODELO 11": (
            "Setup D XAUUSD/M5 | base M8 + inclinacao SMA50 de 1 candle/ATR >= 0,05"
        ),
        "MODELO 12": (
            "Setup E XAUUSD/M5 | ADX + distancia/ATR + inclinacao SMA50"
        ),
        "MODELO 13": (
            "Setup A nos 17 pares Forex/M5 | entrada RSI14/50 com direcao "
            "SMA20/50 | Full Exit RSI 70/30 ou inversao das medias | sem TP"
        ),
        "MODELO 14": (
            "Setup B nos 17 pares Forex/M5 | base M13 + ADX14 > 25"
        ),
        "MODELO 15": (
            "Setup C nos 17 pares Forex/M5 | base M13 + distancia "
            "SMA20/50 por ATR >= 0,25"
        ),
        "MODELO 16": (
            "Setup D nos 17 pares Forex/M5 | base M13 + inclinacao "
            "SMA50 de 1 candle/ATR >= 0,05"
        ),
        "MODELO 17": (
            "Setup E nos 17 pares Forex/M5 | ADX + distancia/ATR + "
            "inclinacao SMA50"
        ),
        "MODELO 18": (
            "M8 melhorado | entrada inicial sem TP | uma reentrada Stop por ciclo "
            "com TP 7,5 pontos | Full Exit fallback"
        ),
        "MODELO 19": (
            "M9 melhorado | base + ADX14 > 25 | entrada inicial sem TP | "
            "uma reentrada com TP 7,5 pontos"
        ),
        "MODELO 20": (
            "M10 melhorado | distancia SMA/ATR >= 0,25 | entrada inicial sem TP | "
            "uma reentrada com TP 7,5 pontos"
        ),
        "MODELO 21": (
            "M11 melhorado | inclinacao SMA50/ATR >= 0,05 | entrada inicial sem TP | "
            "uma reentrada com TP 7,5 pontos"
        ),
        "MODELO 22": (
            "M12 melhorado | ADX + distancia/ATR + inclinacao | entrada inicial "
            "sem TP | uma reentrada com TP 7,5 pontos"
        ),
        "MODELO 23": (
            "Acumulador dos modelos ativos | entrada, SL, TP e saidas herdados "
            "de cada fonte | cesta coletiva | Full Exit a mercado em +US$1.000 liquidos"
        ),
        "MODELO 24": (
    ...
    match = re.fullmatch(r"MODELO (1[5-9]|2[01])", normalized)
    if match is None:
        return ""
...
def _mt5_rows_for_equity_model_filter(
    ...
    return [
        row
        for row in rows
        if _mt5_equity_row_matches_active_model(row, target)
    ]


def _mt5_equity_row_matches_active_model(row: object, model_key: str) -> bool:
    """Keep retired histories out of curves that reuse their model numbers."""
    target = str(model_key or "").upper().replace(" ", "")
    active_xau_models = {
        ...
        "MODELO18": MT5_OPERATIONAL_MODEL_18,
        "MODELO19": MT5_OPERATIONAL_MODEL_19,
        "MODELO20": MT5_OPERATIONAL_MODEL_20,
        "MODELO21": MT5_OPERATIONAL_MODEL_21,
        "MODELO22": MT5_OPERATIONAL_MODEL_22,
    }
    active_xau_model = active_xau_models.get(target)
    if active_xau_model is not None:
        identities = _mt5_equity_row_model_identities(row)
        if not any(active_xau_model in identity for identity in identities):
            return False
        symbol = _mt5_equity_row_symbol(row)
        return symbol == "XAUUSD" or symbol.startswith("XAUUSD.")
    active_forex_models = {
        "MODELO13": MT5_OPERATIONAL_MODEL_13,
        "MODELO14": MT5_OPERATIONAL_MODEL_14,
        "MODELO15": MT5_OPERATIONAL_MODEL_15,
        "MODELO16": MT5_OPERATIONAL_MODEL_16,
        "MODELO17": MT5_OPERATIONAL_MODEL_17,
    }
    active_forex_model = active_forex_models.get(target)
    if active_forex_model is not None:
        identities = _mt5_equity_row_model_identities(row)
        if not any(active_forex_model in identity for identity in identities):
            return False
        symbol = _mt5_equity_row_symbol(row)
        return any(
            symbol == allowed or symbol.startswith(f"{allowed}.")
            for allowed in FOREX_SMA_RSI_PAIRS
        )
    dynamic_models = {
        f"MODELO{spec.number}": spec.model_id
        for spec in (
            dynamic_exit_model_spec(model_id)
            for model_id in DYNAMIC_EXIT_MODEL_IDS
        )
        if spec is not None
    }
    dynamic_model_id = dynamic_models.get(target)
    if dynamic_model_id is not None:
        identities = _mt5_equity_row_model_identities(row)
        return any(dynamic_model_id in identity for identity in identities)
    scoped_models = {
        "MODELO6": (
            MT5_OPERATIONAL_MODEL_6,
            MODEL_6_FOREX_EXPANSION_PAIRS,
    ...
    scoped = scoped_models.get(target)
    if scoped is None:
        return _mt5_equity_row_model_key(row) == target
    model_id, allowed_symbols = scoped
    identities = _mt5_equity_row_model_identities(row)
    if not any(model_id in identity for identity in identities):
        return False
    symbol = _mt5_equity_row_symbol(row)
    return any(
        symbol == allowed or symbol.startswith(f"{allowed}.")
...
def _mt5_equity_row_model_identities(row: object) -> set[str]:
    snapshot = getattr(row, "plan_snapshot", {}) or {}
    if not isinstance(snapshot, dict):
        snapshot = {}
    values = {
        str(getattr(row, "operational_model", "") or "").upper(),
        str(getattr(row, "plan_identity", "") or "").upper(),
        str(snapshot.get("operational_model", "") or "").upper(),
        str(snapshot.get("plan_identity", "") or "").upper(),
    }
    return {value for value in values if value and value not in {"N/D", "NONE"}}


def _mt5_equity_row_symbol(row: object) -> str:
    snapshot = getattr(row, "plan_snapshot", {}) or {}
    if not isinstance(snapshot, dict):
        snapshot = {}
    return str(
        getattr(row, "symbol", "")
        or getattr(row, "mt5_symbol", "")
...
def _mt5_equity_model_filter_caption(
    ...
    visible_numbers = (0, *MT5_ACTIVE_REPORT_MODEL_NUMBERS)
    counts = {f"MODELO{index}": 0 for index in visible_numbers}
    for row in rows:
        key = _mt5_equity_row_model_key(row)
        if key in {"MODELO18", "MODELO19", "MODELO20", "MODELO21", "MODELO22"}:
            if not _mt5_equity_row_matches_active_model(row, key):
                key = "MODELO0"
        model_number = int(key.removeprefix("MODELO")) if key.removeprefix("MODELO").isdigit() else 0
        if model_number and model_number not in MT5_ACTIVE_REPORT_MODEL_NUMBERS:
            continue
        counts[key if key in counts else "MODELO0"] += 1
    details = " | ".join(
        f"M{index}: {counts[f'MODELO{index}']}" for index in visible_numbers
    )
    return (
...
def _mt5_trade_audit_row(
    ...
    return {
        ...
        "Horario pico lucro": str(
            getattr(row, "mt5_peak_open_profit_at", "N/D") or "N/D"
        ),
...
def _model23_theoretical_entry_summary_rows(
    rows_by_source: dict[str, list[dict[str, object]]],
) -> list[dict[str, object]]:
    """Resume as copias M1-M22 sem repetir calculos ou leituras do MT5."""
    summary: list[dict[str, object]] = []
    ordered_sources = sorted(
        rows_by_source,
        key=lambda model_id: operational_model_number(model_id) or 999,
    )
    for source_model_id in ordered_sources:
        source_number = operational_model_number(source_model_id)
        source_rows = list(rows_by_source.get(source_model_id, []) or [])
        ready_pairs: list[str] = []
        waiting = 0
        blocked = 0
        bottlenecks: list[str] = []
        for row in source_rows:
            status = str(row.get("Envio", "AGUARDA: sem leitura") or "").strip()
            normalized = status.upper()
            if normalized.startswith("PRONTO"):
                pair = str(row.get("Par", "N/D") or "N/D").upper()
                if pair not in ready_pairs:
                    ready_pairs.append(pair)
                continue
            if normalized.startswith(("BLOQ", "REJEIT", "ERRO")):
                blocked += 1
            else:
                waiting += 1
            if status and status not in bottlenecks:
                bottlenecks.append(status)
        ready = len(ready_pairs)
        summary.append(
            {
                "Modelo": "M23",
                "Fonte": f"M{source_number}" if source_number is not None else "N/D",
                "Status entrada": f"PRONTO: {ready}" if ready else "AGUARDA",
                "Pares prontos": ", ".join(ready_pairs) if ready_pairs else "NENHUM",
                "Aguardando": waiting,
                "Bloqueados": blocked,
                "Gargalo atual": " | ".join(bottlenecks[:2]) or "SEM_GARGALO",
            }
        )
    return summary


def _model23_theoretical_entry_detail_rows(
    rows_by_source: dict[str, list[dict[str, object]]],
) -> list[dict[str, object]]:
    """Materializa cada ativo das fontes ativas sob a identidade M23."""
    details: list[dict[str, object]] = []
    for source_model_id in MT5_MODEL_23_SOURCE_MODEL_IDS:
        source_number = operational_model_number(source_model_id)
            ...
            source_rows = [
                {
                    "Par": "N/D",
                    "Timeframe": "N/D",
                    "Envio": "AGUARDA: fonte sem dados",
                    "Modelo": "OK: fonte interna M23",
                }
            ]
        for row in source_rows:
            cloned = dict(row)
            source_gate = cloned.pop("Modelo", "OK: fonte interna M23")
            detail = {
                "Modelo": "M23",
                "Fonte": source_label,
                "Par": cloned.pop("Par", "N/D"),
                "Timeframe": cloned.pop("Timeframe", "N/D"),
                "Envio": cloned.pop("Envio", "AGUARDA: sem leitura"),
                "Fonte habilitada": source_gate,
            }
            detail.update(cloned)
    ...
    *,
    ...
    fixed_entry_models = (
        (
            ...
            "Confirma direcao H1 e procura em M15 EMA9/21, ADX14 > 20, "
            "pullback na faixa das medias e fechamento de continuacao.",
        ),
        (
            MT5_OPERATIONAL_MODEL_3,
            "Modelo 3 - XAUUSD M5 RSI50 invertido",
            "Copia M23 do gatilho XAU/M5 invertido pelo RSI14 em torno de 50.",
        ),
        (
            MT5_OPERATIONAL_MODEL_4,
            "Modelo 4 - contexto multi-timeframe",
            "Copia M23 do plano contextual pesquisado no Lab.",
        ),
        (
            MT5_OPERATIONAL_MODEL_5,
        ...
        (
            MT5_OPERATIONAL_MODEL_6,
            "Modelo 6 - expansao Forex Lab",
            "Copia M23 dos vencedores M1 nos pares Forex de expansao.",
        ),
        (
            MT5_OPERATIONAL_MODEL_7,
            "Modelo 7 - Lab Ouro e Bitcoin",
            "Reutiliza o plano vencedor do M1 somente em XAUUSD e BTCUSD.",
        ),
        (
            MT5_OPERATIONAL_MODEL_8,
        ...
        (
            model_id,
            f"Modelo {spec.number} - XAUUSD Setup {spec.setup}",
            "Copia M23 do setup XAU/M5 SMA20/50 + RSI14 com seu filtro proprio.",
        )
        for model_id in XAU_TREND_FILTER_MODEL_IDS
        if (spec := trend_filter_spec(model_id)) is not None
        and model_id not in XAU_IMPROVED_REENTRY_MODEL_IDS
    ) + tuple(
        (
            model_id,
            f"Modelo {spec.number} - Setup {spec.setup} com TP de reentrada",
            (
                f"Deriva do M{spec.number - 10}; entrada inicial sem TP, "
                "uma reentrada Stop com TP fixo de 7,5 pontos e Full Exit fallback."
            ),
        )
        for model_id in XAU_IMPROVED_REENTRY_MODEL_IDS
        if (spec := trend_filter_spec(model_id)) is not None
        ...
        (
            model_id,
            f"Modelo {spec.number} - Forex Setup {spec.setup}",
            "SMA20/50 + RSI14 nos 17 pares Forex/M5 com filtros incrementais.",
        )
        for model_id in FOREX_SMA_RSI_MODEL_IDS
        if (spec := forex_sma_rsi_spec(model_id)) is not None
    )
    dynamic_entry_models = tuple(
        (
            spec.model_id,
            (
                f"Modelo {spec.number} - entrada {spec.source_short_name}, "
                "saida dinamica"
            ),
            (
                f"Reutiliza exatamente o sinal e o risco inicial do "
                f"{spec.source_short_name}; depois permite somente protecao de SL."
            ),
        )
        for spec in (
            dynamic_exit_model_spec(model_id)
        ...
        if spec is not None and spec.model_id in MT5_OPERATIONAL_MODEL_IDS
    )
    model_order = {
        model_id: index
        for index, model_id in enumerate(MT5_OPERATIONAL_MODEL_IDS)
    }
    researched_models = tuple(
        sorted(
            fixed_entry_models + dynamic_entry_models,
            key=lambda spec: model_order.get(spec[0], len(model_order)),
        )
    )
    ...
    for model_id, _title, _caption in researched_models:
        ...
        dynamic_spec = dynamic_exit_model_spec(model_id)
        source_model_id = (
            dynamic_spec.source_model_id if dynamic_spec is not None else model_id
        )
        if source_model_id == MT5_OPERATIONAL_MODEL_1:
            row_builder = lambda source_row: _dynamic_exit_entry_row(
                dict(source_row),
                model_id=model_id,
        ...
        elif source_model_id == MT5_HISTORICAL_XAU_MODEL_16:
            row_builder = lambda source_row: _model16_xau_m5_entry_row(
                service,
                source_row,
                evaluate_live=execution_enabled,
            )
        elif source_model_id == MT5_OPERATIONAL_MODEL_3:
            row_builder = lambda source_row: _model3_xau_m5_rsi50_entry_row(
                service,
                source_row,
                evaluate_live=execution_enabled,
                decision_snapshot=shared_decisions,
            )
        elif source_model_id == MT5_OPERATIONAL_MODEL_6:
            row_builder = lambda source_row: _scoped_m1_entry_row(
                source_row,
                model_label="M6_FOREX_EXPANSION",
                evaluate_live=execution_enabled,
            )
        elif source_model_id == MT5_OPERATIONAL_MODEL_7:
            row_builder = lambda source_row: _scoped_m1_entry_row(
                source_row,
                model_label="M7_XAU_BTC",
                evaluate_live=execution_enabled,
            )
        elif source_model_id in {
            MT5_OPERATIONAL_MODEL_8,
            MT5_OPERATIONAL_MODEL_9,
            MT5_OPERATIONAL_MODEL_10,
            MT5_OPERATIONAL_MODEL_11,
            ...
            *XAU_IMPROVED_REENTRY_MODEL_IDS,
            ...
            row_builder = lambda source_row: _xau_m5_sma_rsi_entry_row(
                service,
                source_row,
        ...
        if dynamic_spec is not None and source_model_id != MT5_OPERATIONAL_MODEL_1:
            source_builder = row_builder
            row_builder = lambda source_row: _dynamic_exit_entry_row(
                source_builder(source_row),
                model_id=model_id,
                evaluate_live=execution_enabled,
            )
        source_rows = _mt5_theoretical_entry_source_rows(
            rows,
        ...
        researched_rows_by_model[model_id] = [
            ...
            for row in source_rows
            if (
                source_model_id not in MT5_SCOPED_M1_OPERATIONAL_MODELS
                and source_model_id != MT5_OPERATIONAL_MODEL_1
                or str(row.get("Par", "") or "").upper()
                in (
                    set(MODEL_6_FOREX_EXPANSION_PAIRS)
                        if source_model_id == MT5_OPERATIONAL_MODEL_6
                        else (
                            set(MODEL_7_ALTERNATIVE_MARKETS)
                            if source_model_id == MT5_OPERATIONAL_MODEL_7
                            else set(MODEL_1_FOREX_PAIRS)
                        )
                )
            )
        ]
    ...
    temporal_source_rows = list(rows)
    for model_rows in researched_rows_by_model.values():
        temporal_source_rows.extend(model_rows)
    temporal_gates = _entry_temporal_gates_by_pair(service, temporal_source_rows)
    model1_entry_rows = [
        ...
        for row in rows
        if str(row.get("Par", "") or "").upper() in set(MODEL_1_FOREX_PAIRS)
    ]
    entry_rows_by_model: dict[str, list[dict[str, object]]] = {
        MT5_OPERATIONAL_MODEL_1: model1_entry_rows,
    }
    for model_id, _title, _caption in researched_models:
        execution_enabled = _mt5_operational_model_enabled(
            operational_model,
    ...
    if operational_model in {
        ...
        st.subheader("Entrada Teorica MT5 - Modelo 23")
        st.caption(
            "Detalhamento completo das copias internas M1-M22: uma linha por "
            "fonte e ativo. Os calculos sao reutilizados sem nova consulta ao MT5."
        )
        _render_stable_readonly_table(
            _model23_theoretical_entry_detail_rows(entry_rows_by_model),
            model_column="Fonte",
    ...
    for model_id, title, caption in researched_models:
        ...
        if model_id in FOREX_SMA_RSI_MODEL_IDS:
            st.caption(
                f"{caption} Sem TP fixo; reentrada Stop e Full Exit RSI 70/30."
            )
        else:
            st.caption(
...
def _mt5_theoretical_entry_source_rows(
    ...
    *,
    ...
    """Mantem modelos XAU/M5 visiveis sem herdar linhas Forex/H1 do M1."""
    source_rows = [dict(row) for row in rows]
    if source_model_id in FOREX_SMA_RSI_MODEL_IDS:
        by_pair = {
        ...
        return [
            {
                **dict(by_pair.get(pair, {"Par": pair, "Direcao": "WAIT"})),
                "Par": pair,
                "TF": "M5",
                "Timeframe": "M5",
                "Periodo de tempo": "M5",
    ...
    xau_rows = [
        row
        for row in source_rows
        if str(row.get("Par", "") or "").upper() == "XAUUSD"
    ]
    if not xau_rows:
        xau_rows = [
            {
    ...
    selected["TF"] = "M5"
    selected["Timeframe"] = "M5"
    selected["Periodo de tempo"] = "M5"
    return [selected]


def _forex_m5_sma_rsi_entry_row(
    service: DashboardService,
    row: dict[str, object],
    *,
    model_id: str,
    ...
    """Projeta M13-M17 com o mesmo avaliador usado pelo executor."""
    cloned = dict(row)
    pair = str(cloned.get("Par", "") or "").upper()
    spec = forex_sma_rsi_spec(model_id)
    shared = dict(decision_snapshot or {})
    decision = shared.get((model_id, pair))
    candle_count = int(shared.get(("__FOREX_M5__", pair), 0) or 0)
    if decision is None:
        candles = list(
        ...
        decision = evaluate_forex_sma_rsi_entry(
            model_id, pair, candles,
            awaiting_reentry_side=str(runtime.get("entry_intent_side") or ""),
        )
        decision = service._block_seed_only_forex_m5_decision(decision, pair)
    diagnostics = (
        f"SMA20={decision.base.sma20 if decision.base.sma20 is not None else 'N/D'} | "
        f"SMA50={decision.base.sma50 if decision.base.sma50 is not None else 'N/D'} | "
        f"RSI14={decision.base.rsi14 if decision.base.rsi14 is not None else 'N/D'} | "
    ...
    cloned.update(
        {
            "Par": pair, "TF": "M5", "Timeframe": "M5",
            "Periodo de tempo": "M5",
            "Modelo ativo": f"M{spec.number if spec else '?'}_SETUP_{spec.setup if spec else '?'}",
            "Direcao": decision.base.direction if evaluate_live and decision.ready else "WAIT",
            "Direcao Teorica": decision.base.direction if decision.ready else "WAIT",
            "Plano Research": decision.status,
            "Motivo": decision.reason,
            "Motivo Entrada": decision.reason,
            "Status indicadores": decision.status,
            "Leitura indicadores": diagnostics,
            "Candles recebidos": max(candle_count - 1, 0),
            "Candle Gatilho": decision.base.closed_candle_time,
            "Candle do Sinal": decision.base.closed_candle_time,
            "Candle atual modelo": decision.base.current_candle_time,
            "Horario": decision.base.current_candle_time,
            "RSI14": decision.base.rsi14,
            "SMA20": decision.base.sma20,
            "SMA50": decision.base.sma50,
            "ADX14": decision.adx14,
            "Distancia ATR": decision.distance_atr,
            "Inclinacao SMA50 ATR": decision.sma50_slope_atr,
            "Entrada": decision.base.entry_price,
            "Preco Teorico": decision.base.entry_price,
            "Stop": decision.base.initial_stop,
            "Stop Research": decision.base.initial_stop,
            "Alvo Research": "SEM_TP_FIXO",
            "Codigo Rejeicao": "N/D" if decision.ready else decision.status,
        }
    )
    return cloned


def _model3_xau_m5_rsi50_entry_row(
    service: DashboardService,
    row: dict[str, object],
    *,
    evaluate_live: bool,
    ...
    """Projeta o M3 com a mesma decisao XAUUSD/M5 usada pelo executor."""
    cloned = dict(row)
    shared = dict(decision_snapshot or {})
    decision = shared.get((MT5_OPERATIONAL_MODEL_3, "XAUUSD"))
    if decision is None:
        decision = service.get_model3_entry_decision()
    ...
    parameters = {**model3_parameters(), "family": "XAU_RSI50_SMA20_FLIP"}
    ready = bool(decision.ready)
    direction = str(decision.direction if ready else "WAIT").upper()
    status = str(decision.status or "AGUARDA")
    reason = str(decision.reason or status)
    diagnostics = (
        ...
        f"CLOSE={decision.closed_price if decision.closed_price is not None else 'N/D'}"
    )
    cloned.update(
        {
            "_Modelo Base Raw": row.get("Modelo Ativo", ""),
            "_Parametros Lab Raw": parameters,
            "Modelo Ativo": "M3_XAU_M5_RSI14_FLIP",
            "Modelo Saida": MODEL_3_BETA_ID,
            "Alpha Lab": MODEL_3_ALPHA_ID,
            "Beta Lab": MODEL_3_BETA_ID,
            "Gestao Stop": MODEL_3_STOP_MANAGEMENT,
            "TF": "M5",
            "Timeframe": "M5",
            "Periodo de tempo": "M5",
            "Fonte Lab": "MODEL_3_MANUAL_RULE",
            "Familia Lab": "XAU_RSI50_SMA20_FLIP",
            ...
            "Preco Teorico": _format_model2_price(decision.entry_price or 0.0),
            "Stop Research": _format_model2_price(decision.initial_stop or 0.0),
            "Alvo Research": "SEM_TP_FIXO",
            "RR Research": "REVERSAO_RSI50",
            "RR Minimo": "N/D",
            "Proxima Tentativa": "Recalcular no proximo candle M5 fechado.",
            "Gatilho Esperado": status,
        }
    )
        ...
        cloned.update(
            {
                ...
                "Status indicadores": "MODELO_NAO_SELECIONADO",
                "Motivo Entrada": "Selecione M3 ou Todos para acompanhar XAUUSD/M5.",
            }
        )
    elif ready:
        cloned.update(
...
def _xau_m5_sma_rsi_entry_row(
    ...
    *,
    ...
    """Projeta M8-M12 com a mesma decisao XAUUSD/M5 usada pelo executor."""
    cloned = dict(row)
    spec = trend_filter_spec(model_id)
    shared = dict(decision_snapshot or {})
    shared_decision = shared.get((model_id, "XAUUSD"))
    candle_count = int(shared.get(("__XAU_M5__", "CANDLE_COUNT"), 0) or 0)
    if candle_count <= 0:
        candle_count = len(
            list(
    ...
    if model_id == MT5_OPERATIONAL_MODEL_8:
        decision = shared_decision or service.get_model8_entry_decision()
        base = decision
        model_number = 8
        setup = "A"
        ...
        extra_readings: tuple[tuple[str, object], ...] = ()
    elif spec is not None:
        decision = shared_decision or service.get_xau_trend_filter_entry_decision(
            model_id
        )
        base = decision.base
        model_number = spec.number
        setup = spec.setup
        ...
        extra_readings = (
            ("ADX14", decision.adx14),
            ("ATR14", decision.atr14),
            ("DISTANCE_ATR", decision.distance_atr),
            ("SMA50_SLOPE_ATR", decision.sma50_slope_atr),
        )
    else:
        return cloned
    ...
    parameters = {
        **parameters,
        "family": "XAU_SMA_RSI_TREND_FILTER",
    }
    readings = (
        ("SMA20", base.sma20),
        ("SMA50", base.sma50),
    ...
    diagnostics = " | ".join(
        f"{name}={value if value is not None else 'N/D'}"
        for name, value in readings
    )
    ready = bool(decision.ready)
    direction = str(base.direction if ready else "WAIT").upper()
    status = str(decision.status or "AGUARDA")
    reason = str(decision.reason or status)
    cloned.update(
        {
            "_Modelo Base Raw": row.get("Modelo Ativo", ""),
            "_Parametros Lab Raw": parameters,
            "Modelo Ativo": f"M{model_number}_XAU_M5_SETUP_{setup}",
            "Modelo Saida": beta_id,
            "Alpha Lab": alpha_id,
            "Beta Lab": beta_id,
            "Gestao Stop": stop_management,
            "TF": "M5",
            "Timeframe": "M5",
            "Periodo de tempo": "M5",
            "Fonte Lab": source,
            "Familia Lab": "XAU_SMA_RSI_TREND_FILTER",
            "Paridade Demo": "APROVADA",
            "Parametros Lab": " | ".join(
                f"{key}={value}" for key, value in parameters.items()
            ),
            "Candle Gatilho": base.closed_candle_time,
            "Candle do Sinal": base.closed_candle_time,
            "Candle atual modelo": base.current_candle_time,
            "Horario": base.current_candle_time,
            "Candles recebidos": max(candle_count - 1, 0),
            "Status indicadores": status,
            "Leitura indicadores": diagnostics,
            "RSI14": base.rsi14,
            ...
            "SMA50": base.sma50,
            "Motivo Entrada": reason,
            "Preco Teorico": _format_model2_price(base.entry_price or 0.0),
            "Stop Research": _format_model2_price(base.initial_stop or 0.0),
            "Alvo Research": "SEM_TP_FIXO",
            "RR Research": "DINAMICO_POR_SL",
            "RR Minimo": "N/D",
            "Proxima Tentativa": "Recalcular no proximo candle M5 fechado.",
            "Gatilho Esperado": status,
        }
        ...
        cloned.update(
            {
                ...
                "Status indicadores": "MODELO_NAO_SELECIONADO",
                "Motivo Entrada": (
                    f"Selecione M{model_number} ou Todos para acompanhar XAUUSD/M5."
                ),
            }
        )
    elif ready:
...
def _model15_xau_m5_entry_row(
    service: DashboardService,
    row: dict[str, object],
    *,
    evaluate_live: bool,
) -> dict[str, object]:
    """Projeta a mesma leitura M5 consumida pelo executor do M15."""
    cloned = dict(row)
    decision = service.get_model15_entry_decision(read_only_fallback=True)
    diagnostics = (
        f"EMA20={_format_model2_price(decision.ema20 or 0.0)} | "
        f"EMA50={_format_model2_price(decision.ema50 or 0.0)} | "
        f"max anterior={_format_model2_price(decision.previous_high or 0.0)} | "
        f"min anterior={_format_model2_price(decision.previous_low or 0.0)}"
    )
    cloned.update(
        {
            "_Modelo Base Raw": row.get("Modelo Ativo", ""),
            "Modelo Ativo": "M15_XAU_M5_EMA_BREAKOUT_TRAILING",
            "Modelo Saida": MODEL_15_BETA_VERSION,
            "Alpha Lab": MODEL_15_ALPHA_ID,
            "Beta Lab": MODEL_15_BETA_ID,
            "Gestao Stop": MODEL_15_STOP_MANAGEMENT,
            "TF": "M5",
            "Timeframe": "M5",
            "Periodo de tempo": "M5",
            "Fonte Lab": "M15_MANUAL_RULE",
            "Familia Lab": "XAU_EMA_BREAKOUT",
            "Paridade Demo": "APROVADA",
            "Parametros Lab": (
                "EMA20/50 | entrada=extremo anterior+1pip | "
                "SL=extremo oposto exato | sem TP fixo"
            ),
            "Candle Gatilho": decision.previous_candle_time,
            "Candle do Sinal": decision.previous_candle_time,
            "Candle atual modelo": decision.current_candle_time,
            "Status indicadores": decision.status,
            "Leitura indicadores": diagnostics,
            "Motivo Entrada": decision.reason,
            ...
            "Proxima Tentativa": "Recalcular no proximo candle M5 fechado.",
            "Gatilho Esperado": (
                f"{decision.direction} STOP no extremo anterior"
                if decision.ready
        ...
        cloned.update(
            {
                ...
                "Codigo Rejeicao": "SELECIONE_MODELO_OU_TODOS",
                "Status indicadores": "MODELO_NAO_SELECIONADO",
                "Motivo Entrada": "Selecione M15 ou Todos para acompanhar XAUUSD/M5.",
            }
        )
        return cloned
    if decision.ready:
        cloned.update(
            {
                "Entrada Teorica": "SINAL_TEORICO",
                "Direcao Teorica": decision.direction,
                "Direcao": "COMPRAR" if decision.direction == "BUY" else "VENDER",
                "Plano Research": "PLANO_VALIDO",
                "Codigo Rejeicao": "N/D",
            }
        )
...
def _model16_xau_m5_entry_row(
    service: DashboardService,
    row: dict[str, object],
    *,
    evaluate_live: bool,
) -> dict[str, object]:
    """Projeta a mesma leitura M5 consumida pelo executor do M16."""
    cloned = dict(row)
    decision = service.get_model16_entry_decision(read_only_fallback=True)
    diagnostics = (
        f"preco={_format_model2_price(decision.current_price or 0.0)} | "
        f"EMA20={_format_model2_price(decision.ema20 or 0.0)} | "
        f"max anterior={_format_model2_price(decision.previous_high or 0.0)} | "
        f"min anterior={_format_model2_price(decision.previous_low or 0.0)}"
    )
    cloned.update(
        {
            "_Modelo Base Raw": row.get("Modelo Ativo", ""),
            "Modelo Ativo": "M16_XAU_M5_PRICE_EMA20_BREAKOUT_TRAILING",
            "Modelo Saida": MODEL_16_BETA_VERSION,
            "Alpha Lab": MODEL_16_ALPHA_ID,
            "Beta Lab": MODEL_16_BETA_ID,
            "Gestao Stop": MODEL_16_STOP_MANAGEMENT,
            "TF": "M5",
            "Timeframe": "M5",
            "Periodo de tempo": "M5",
            "Fonte Lab": "M16_MANUAL_RULE",
            "Familia Lab": "XAU_PRICE_EMA20_BREAKOUT",
            "Paridade Demo": "APROVADA",
            "Parametros Lab": (
                "preco versus EMA20 | entrada=extremo anterior+1pip | "
                "SL=extremo oposto exato | sem TP fixo"
            ),
            "Candle Gatilho": decision.previous_candle_time,
            "Candle do Sinal": decision.previous_candle_time,
        ...
        cloned.update(
            {
                ...
                "Codigo Rejeicao": "SELECIONE_MODELO_OU_TODOS",
                "Status indicadores": "MODELO_NAO_SELECIONADO",
                "Motivo Entrada": "Selecione M16 ou Todos para acompanhar XAUUSD/M5.",
            }
        )
        return cloned
    if decision.ready:
...
def _lab_operational_entry_row(
    ...
    *,
    ...
    effective_demo_forward_enabled = (
        demo_forward_enabled or allow_demo_forward_override
    )
    ...
    cloned["Paridade Demo"] = (
        "APROVADA_M23"
        if allow_demo_forward_override and not demo_forward_enabled
        else "APROVADA"
        if demo_forward_enabled
        else "BLOQUEADA"
    )
    ...
    if not effective_demo_forward_enabled:
...
def _scoped_m1_entry_row(
    row: dict[str, object],
    *,
    model_label: str,
    evaluate_live: bool,
) -> dict[str, object]:
    """Expose the existing M1 Lab reading under an isolated market group."""
    cloned = dict(row)
    plan_status = str(row.get("Plano Research", "") or "").upper()
    direction = str(row.get("Direcao Teorica", "WAIT") or "WAIT").upper()
    ready = plan_status == "PLANO_VALIDO" and direction in {"BUY", "SELL"}
    cloned.update(
        {
            "_Modelo Base Raw": row.get("Modelo Ativo", ""),
            "Modelo Ativo": model_label,
            "Fonte Lab": "M1",
            "Paridade Demo": "APROVADA",
            "Status indicadores": "READY" if ready else "AGUARDA_SINAL_M1",
            "Leitura indicadores": _mt5_m1_live_indicator_summary(row),
            "Candle atual modelo": row.get("Horario", "N/D"),
            "Motivo Entrada": (
                "Plano vencedor do Lab M1 pronto neste grupo."
                if ready
                else str(row.get("Motivo Entrada", "Aguardando plano M1."))
            ),
        }
    )
    if not evaluate_live:
...
def _dynamic_exit_entry_row(
    source_row: dict[str, object],
    *,
    model_id: str,
    evaluate_live: bool,
) -> dict[str, object]:
    """Relabel a source entry without recalculating any market indicator."""
    cloned = dict(source_row)
    spec = dynamic_exit_model_spec(model_id)
    if spec is None:
        return cloned
    source_status = str(cloned.get("Plano Research", "") or "").upper()
    source_direction = str(
        cloned.get("Direcao Teorica", cloned.get("Direcao", "WAIT")) or "WAIT"
    ).upper()
    ready = source_status == "PLANO_VALIDO" and source_direction in {
        "BUY",
        "SELL",
        "COMPRAR",
        "VENDER",
    ...
    cloned.update(
        {
            "_Modelo Base Raw": source_row.get("Modelo Ativo", ""),
            "Modelo Ativo": (
                f"{spec.short_name}_DYNAMIC_FROM_{spec.source_short_name}"
            ),
            "Modelo Saida": spec.beta_version,
            "Beta Lab": spec.beta_id,
            "Modo Beta": "DYNAMIC_PROTECT_ONLY",
            "Gestao Stop": DYNAMIC_EXIT_POLICY,
            "Fonte Config": (
                f"ENTRY={spec.source_short_name} | EXIT=POSITION_MANAGER"
            ),
            "Motivo Entrada": (
                f"Entrada copiada integralmente do {spec.source_short_name}; "
                "somente a gestao posterior foi alterada."
                if ready
                else str(
                    source_row.get(
                        "Motivo Entrada",
                        f"Aguardando entrada valida do {spec.source_short_name}.",
                    )
                )
            ),
        }
...
def _mt5_model_indicator_monitor_rows(
    ...
    *,
    ...
    if selected == MT5_OPERATIONAL_MODEL_ALL:
        selected_models = MT5_OPERATIONAL_MODEL_IDS
    elif selected in {
        MT5_OPERATIONAL_MODEL_CUSTOM,
        MT5_OPERATIONAL_MODEL_WITH_23,
    }:
        selected_models = _load_persisted_mt5_operational_models()
    elif selected == MT5_OPERATIONAL_MODEL_23:
        selected_models = MT5_MODEL_23_SOURCE_MODEL_IDS
    elif selected == MT5_OPERATIONAL_MODEL_8_TO_17:
        selected_models = MT5_OPERATIONAL_MODEL_8_TO_17_IDS
    else:
...
def _mt5_expected_model_indicators(
    ...
    *,
    ...
    official_indicators = {
        ...
        "XAU_RSI50_SMA20_FLIP": ("RSI14", "SMA20", "CLOSE"),
        "XAU_SMA_RSI_TREND_FILTER": (
            "SMA20",
            "SMA50",
            "RSI14",
            "ADX14",
    ...
    if family == "TREND_PULLBACK_M15_H1":
        return (
            f"M15_EMA{parameters.get('fast', 9)}",
            f"M15_EMA{parameters.get('slow', 21)}",
            f"ADX{parameters.get('adx_period', 14)}",
            f"ATR{parameters.get('atr_period', 14)}",
            "PULLBACK_TOUCH",
            "CONFIRM_BULLISH",
            "CONFIRM_BEARISH",
            f"H1_EMA{parameters.get('context_fast', 20)}",
            f"H1_EMA{parameters.get('context_slow', 50)}",
            "H1_TREND",
            "M15_TREND",
            "M2_SIGNAL",
        )
...
def _mt5_indicator_lab_rule(
    ...
    *,
    ...
    family = str(parameters.get("family") or "").upper()
    keys: tuple[str, ...]
    if family == "TREND_PULLBACK_M15_H1" and normalized.startswith("M15_EMA"):
        keys = ("fast", "slow")
    elif family == "TREND_PULLBACK_M15_H1" and normalized.startswith("H1_EMA"):
        keys = ("context_fast", "context_slow")
    elif family == "TREND_PULLBACK_M15_H1" and normalized.startswith("ADX"):
        keys = ("adx_period", "adx_min")
    elif family == "TREND_PULLBACK_M15_H1" and normalized.startswith("ATR"):
        keys = ("atr_period", "stop_factor", "risk_reward")
    elif family == "TREND_PULLBACK_M15_H1" and normalized == "PULLBACK_TOUCH":
        keys = ("pullback_rule",)
    elif family == "TREND_PULLBACK_M15_H1" and normalized.startswith("CONFIRM"):
        keys = ("confirmation_rule",)
    elif normalized.startswith("EMA"):
...
def _exibir_saidas_teoricas_mt5(
    ...
    st.caption(
        ...
        "na ordem. M1-M7 preservam SL/TP fixos. M8-M17 usam Full Exit por "
        "regra propria. M18-M22 usam TP apenas na reentrada e Full Exit fallback por "
        "cruzamento fechado RSI 70/30 ou inversao SMA20/SMA50. Os antigos M8-M14 reutilizavam as "
        "entradas de origem e permanecem somente no historico. Posicoes "
        "legadas continuam identificadas pelo contrato gravado quando abertas. "
        "A coluna Selecao operacional informa quais modelos podem abrir novas "
        "ordens e quais aparecem apenas para manter a gestao da posicao anterior."
    )
    st.markdown(
        (
            "<div class='traderia-status-legend'>"
    ...
    selected_for_new_entries = set(_load_persisted_mt5_operational_selections())
    rows = [
        _mt5_theoretical_exit_row(
            ...
            selected_for_new_entries=selected_for_new_entries,
        )
...
def _mt5_theoretical_exit_row(
    ...
    *,
    ...
    selected_for_new_entries: set[str] | None = None,
) -> dict[str, object]:
    ...
    return {
        ...
        "Modelo": model_label,
        "Selecao operacional": _mt5_theoretical_exit_selection_status(
            effective_model,
            selected_for_new_entries,
        ),
...
def _mt5_theoretical_exit_selection_status(
    effective_model: str,
    selected_for_new_entries: set[str] | None,
) -> str:
    """Distingue autorizacao de entrada da gestao obrigatoria da posicao aberta."""
    if selected_for_new_entries is None:
        return "N/D"
    model = str(effective_model or "").upper()
    for selected in selected_for_new_entries:
        candidate = str(selected or "").upper()
        if model == candidate or model.startswith(f"{candidate}_SOURCE_"):
            return "ATIVO PARA ENTRADA"
    return "SOMENTE GESTAO (POSICAO ANTERIOR)"
...
def _mt5_theoretical_exit_scenario_label(
    ...
    if display_model in MT5_DYNAMIC_EXIT_OPERATIONAL_MODELS:
        return "DYNAMIC_PROTECT_ONLY"
...
def _mt5_sender_model_label(
    ...
    if "MODELO_9" in model or model in {"M9", "MODELO 9"}:
        return "MODELO 9"
    if "MODELO_8" in model or model in {"M8", "MODELO 8"}:
        return "MODELO 8"
    ...
    if model == MT5_OPERATIONAL_MODEL_7:
        return "MODELO 7"
    if model == MT5_OPERATIONAL_MODEL_8:
        return "MODELO 8"
    if model == MT5_OPERATIONAL_MODEL_9:
        return "MODELO 9"
    if model == MT5_OPERATIONAL_MODEL_10:
        return "MODELO 10"
...
def _mt5_theoretical_exit_cell_class(row: dict[str, object], column: str) -> str:
    """Destaca a celula dos modelos ativos e preserva cores historicas."""
    if column == "Modelo envio":
        ...
        if model == "MODELO 8":
            return "traderia-cell-model8"
        if model == "MODELO 9":
            return "traderia-cell-model9"
        if model == "MODELO 10":
            return "traderia-cell-model10"
        if model == "MODELO 3":
...
def _mt5_theoretical_exit_model_row_class(row: dict[str, object]) -> str:
    ...
    if "MODELO 9" in model_text or "MODELO9" in model_text or "M9" in model_text:
        return "traderia-row-model9"
    if "MODELO 8" in model_text or "MODELO8" in model_text or "M8" in model_text:
        return "traderia-row-model8"
...
def _forex_theoretical_entry_row(
    ...
    *,
    ...
    manual_rule_model = _entry_is_manual_rule_model(operational_model)
    previous_candle_pending = operational_model in {
        MT5_HISTORICAL_XAU_MODEL_15,
        MT5_HISTORICAL_XAU_MODEL_16,
    }
    ...
    zone_gate = (
        (
            "OK: nao aplicavel ao M16"
            if operational_model == MT5_HISTORICAL_XAU_MODEL_16
            else "OK: nao aplicavel ao M15"
        )
        if previous_candle_pending
        else (
    ...
    if manual_rule_model:
        zone_gate = "OK: nao aplicavel; regra incorporada ao setup"
    robot_gate = _entry_robot_gate(robot_online)
    mt5_gate = _entry_mt5_gate(mt5_online)
    if previous_candle_pending:
        if operational_model == MT5_HISTORICAL_XAU_MODEL_16:
            filter_gate = "OK: preco/EMA20 e extremo M5"
            regime_gate = "OK: direcao definida por preco versus EMA20"
        else:
            filter_gate = "OK: EMA20/50 e extremo M5"
            regime_gate = "OK: direcao definida pela EMA20/50"
    elif lab_operational:
        filter_gate = "OK: parametros do modelo"
        regime_gate = "OK: incorporado ao sinal Lab"
    elif manual_rule_model:
        filter_gate = "OK: filtros incorporados ao setup"
        regime_gate = "OK: direcao definida pelo proprio setup"
    else:
        filter_gate = (
    ...
    price_gate = (
        _entry_price_gate_for_model15(row)
        if previous_candle_pending
        else _entry_price_gate(row, operational_model=operational_model)
    )
    ...
    return {
        ...
        "Indicadores atuais": row.get("Leitura indicadores", "N/D"),
        "RSI14": row.get("RSI14", "N/D"),
        "SMA20": row.get("SMA20", "N/D"),
        "SMA50": row.get("SMA50", "N/D"),
...
def _mt5_open_position_context_by_model(
    ...
    for position in list(getattr(report, "rows", []) or []):
        ...
        model_key = _mt5_equity_row_model_key(position)
        model_id = model_ids.get(model_key)
        if model_key in {"MODELO3", "MODELO8", "MODELO9", "MODELO10", "MODELO11", "MODELO12"}:
            identities = _mt5_equity_row_model_identities(position)
            if model_id is None or not any(model_id in value for value in identities):
                continue
        side = str(
...
def _entry_is_manual_rule_model(operational_model: str) -> bool:
    """Modelos cuja Alpha ja contem todos os filtros de entrada necessarios."""
    return str(operational_model or "").upper() in {
        MT5_OPERATIONAL_MODEL_3,
        MT5_OPERATIONAL_MODEL_8,
        MT5_OPERATIONAL_MODEL_9,
...
def _entry_demo_release_gate(
    ...
    if _entry_is_manual_rule_model(operational_model) or operational_model in {
        MT5_HISTORICAL_XAU_MODEL_15,
        MT5_HISTORICAL_XAU_MODEL_16,
    }:
        return "OK: regra manual autorizada Demo"
    ...
    if status in {"APROVADA", "APROVADA_M23"}:
        return (
            "OK: liberado para copia M23 Demo"
            if status == "APROVADA_M23"
            else "OK: par liberado Demo"
        )
...
def _entry_data_gate(
    ...
    if not _entry_is_lab_operational_model(operational_model):
        ...
        candle_reference = next(
            (
                str(row.get(key, "") or "").strip()
                for key in ("Horario", "Candle atual modelo", "Candle do Sinal")
                if str(row.get(key, "") or "").strip().upper()
                not in {"", "N/D", "NONE"}
            ),
            "",
        )
        if received > 0 and candle_reference:
...
def _entry_indicators_gate(
    ...
    if _entry_is_manual_rule_model(operational_model):
        readings = str(row.get("Leitura indicadores", "") or "").strip().upper()
        status = str(row.get("Status indicadores", "") or "").strip().upper()
        if readings not in {"", "N/D", "SEM_LEITURA"}:
            return "OK: parametros do setup avaliados"
        return f"AGUARDA: {status or 'indicadores do setup'}"
    if not _entry_is_lab_operational_model(operational_model):
        return "OK: leitura do timeframe calculada"
    status = str(row.get("Status indicadores", "") or "").upper()
    if not status and str(row.get("Entrada Teorica", "") or "").upper() == "SINAL_TEORICO":
...
def _entry_window_gate(
    ...
    if not _entry_is_lab_operational_model(operational_model):
        label = "sinal do setup" if _entry_is_manual_rule_model(operational_model) else "sinal M1"
        return "OK" if _gate_status(entry_gate) == "ok" else f"AGUARDA: {label}"
    status = str(row.get("Status indicadores", "") or "").upper()
...
def _entry_mt5_gate(mt5_online: bool) -> str:
    return "OK" if mt5_online else "AGUARDA: conector MT5 sem leitura"
...
def _entry_price_gate(
    ...
    *,
    ...
    if direction not in {"BUY", "SELL"}:
        status = str(
            row.get("Status indicadores")
            or row.get("Codigo Rejeicao")
            or "sem direcao"
        ).strip()
        return f"AGUARDA: {status}"
    if _entry_is_manual_rule_model(operational_model):
        ...
        if direction == "SELL" and reference_price < stop:
            return "OK: sem TP fixo; Full Exit pelo setup"
        return f"BLOQ: SL invalido para {direction}"
...
def _entry_price_gate_for_model15(row: dict[str, object]) -> str:
    """Valida o desenho da ordem STOP M15/M16 sem exigir TP fixo."""
    direction = str(row.get("Direcao Teorica", "") or "").upper()
    trigger = _price_from_display(row.get("Preco Teorico"))
    stop = _price_from_display(row.get("Stop Research"))
    last_price = _price_from_display(row.get("Ultimo preco"))
    if direction not in {"BUY", "SELL"}:
        return "AGUARDA: sem direcao STOP"
    if trigger is None or stop is None:
        return "AGUARDA: gatilho/SL STOP ausente"
    if direction == "BUY" and stop >= trigger:
        return "BLOQ: SL invalido para BUY STOP"
    if direction == "SELL" and stop <= trigger:
        return "BLOQ: SL invalido para SELL STOP"
    if last_price is None:
        return "OK: validacao final no broker MT5"
    if direction == "BUY" and trigger <= last_price:
        return "BLOQ: gatilho BUY STOP ja rompido"
    if direction == "SELL" and trigger >= last_price:
        return "BLOQ: gatilho SELL STOP ja rompido"
    return f"OK: {direction} STOP aguardando rompimento"
...
def _read_mt5_demo_execution_records() -> list[dict[str, object]]:
    try:
        lines = read_last_text_lines(MT5_DEMO_EXECUTION_LOG_PATH, limit=2000)
    except OSError:
        return []
...
def _mt5_trade_audit_model_row_class(row: dict[str, object]) -> str:
    ...
    if "MODELO 9" in model or "MODELO9" in model or model == "M9":
        return "traderia-row-model9"
    if "MODELO 8" in model or "MODELO8" in model or model == "M8":
        return "traderia-row-model8"
...
def _inject_dashboard_css() -> None:
    st.markdown(
        ...
        .traderia-row-model8 td {
            background: #E0F7FA !important;
            color: #164E63 !important;
            font-weight: 800;
        }
        .traderia-row-model9 td {
            background: #FFEDD5 !important;
            color: #7C2D12 !important;
            font-weight: 800;
        }
        .traderia-row-model10 td {
        ...
        .traderia-stable-table td.traderia-cell-model8 {
            background: #A5F3FC !important;
            color: #164E63 !important;
            font-weight: 900;
        }
        .traderia-stable-table td.traderia-cell-model9 {
            background: #FED7AA !important;
            color: #7C2D12 !important;
            font-weight: 900;
        }
        .traderia-stable-table td.traderia-cell-model10 {
        ...
        .traderia-legend-model8 {
            background: #E0F7FA;
            color: #164E63;
        }
        .traderia-legend-model9 {
            background: #FFEDD5;
            color: #7C2D12;
        }
        .traderia-legend-model10 {
            background: #E5E7EB;
            color: #1F2937;
        }
...
def exibir_replay_forex_pair_dashboard(
    ...
    _exibir_model_exit_research(service)
...
def _exibir_model_exit_research(service: DashboardService) -> None:
    """Show the persisted heavy study without recalculating it on reruns."""
    loader = getattr(service, "get_model_exit_research", None)
    payload = loader() if callable(loader) else {}
    if not isinstance(payload, dict) or not payload:
        st.info(
            "Pesquisa de stop/alvo dos modelos M8-M10 ainda nao foi gerada."
        )
        return
    models = dict(payload.get("models") or {})
    model_options = [item for item in ("M8", "M9", "M10") if item in models]
    if not model_options:
        return

    st.markdown("### Pesquisa de stop e alvo por modelo")
    st.caption(
        "Resultado pesado persistido: esta secao apenas le o arquivo da "
        "pesquisa e nao participa do ciclo automatico Forex."
    )
    selected = st.selectbox(
        "Modelo pesquisado",
        model_options,
        key="replay_exit_research_model",
        format_func=lambda value: {
            "M8": "M8 - direcao H1 / entrada M5",
            "M9": "M9 - direcao M15 / entrada M1",
            "M10": "M10 - direcao D1 / entrada M15",
        }.get(value, value),
    )
    model = dict(models.get(selected) or {})
    winner = dict(model.get("global_winner") or {})
    metrics = st.columns(6)
    metrics[0].metric("Modelo", selected)
    metrics[1].metric("Stop vencedor", f"{float(winner.get('stop_factor', 0.0)):.2f} ATR")
    metrics[2].metric("Alvo vencedor", f"{float(winner.get('risk_reward', 0.0)):.2f}R")
    metrics[3].metric("Trades", int(winner.get("trades", 0) or 0))
    metrics[4].metric("Resultado", f"{float(winner.get('net_r', 0.0)):.2f}R")
    metrics[5].metric("Profit factor", f"{float(winner.get('profit_factor', 0.0)):.2f}")
    st.caption(
        f"Gerado em {payload.get('generated_at', 'N/D')} | "
        f"Base: {int(payload.get('candle_limit', 0) or 0)} candles por par | "
        "Resultado bruto, sem spread, comissao ou swap."
    )
    if float(winner.get("net_r", 0.0) or 0.0) <= 0:
        st.warning(
            "Nenhuma combinacao unica de stop/alvo foi positiva no agregado "
            "dos oito pares deste modelo. Os vencedores individuais abaixo "
            "continuam sendo evidencia de pesquisa, nao promocao operacional."
        )

    pair_rows = [
        _model_exit_research_pair_row(selected, pair, pair_data)
        for pair, pair_data in sorted(dict(model.get("pairs") or {}).items())
    ]
    st.markdown("#### Melhor combinacao por par")
    _render_stable_readonly_table(pair_rows)

    ranking_rows = [
        _model_exit_research_ranking_row(item)
        for item in list(model.get("aggregate_ranking") or [])
    ]
    if ranking_rows:
        st.markdown("#### Mapa agregado das combinacoes")
        st.vega_lite_chart(
            ranking_rows,
            _model_exit_research_heatmap_spec(),
            use_container_width=True,
        )
        with st.expander("Ranking completo do modelo", expanded=False):
            _render_stable_readonly_table(ranking_rows)


def _model_exit_research_pair_row(
    model: str,
    pair: str,
    pair_data: object,
) -> dict[str, object]:
    data = dict(pair_data or {})
    winner = dict(data.get("winner") or {})
    return {
        "Modelo": model,
        "Par": str(pair),
        "Direcao TF": str(data.get("context_timeframe") or "N/D"),
        "Entrada TF": str(data.get("entry_timeframe") or "N/D"),
        "Stop vencedor": f"{float(winner.get('stop_factor', 0.0)):.2f} ATR",
        "Alvo vencedor": f"{float(winner.get('risk_reward', 0.0)):.2f}R",
        "Trades": int(winner.get("trades", 0) or 0),
        "Vitorias": int(winner.get("wins", 0) or 0),
        "Derrotas": int(winner.get("losses", 0) or 0),
        "Win rate": _optional_percent(float(winner.get("win_rate", 0.0) or 0.0)),
        "Profit factor": f"{float(winner.get('profit_factor', 0.0)):.2f}",
        "Expectativa": f"{float(winner.get('expectancy_r', 0.0)):.3f}R",
        "Resultado": f"{float(winner.get('net_r', 0.0)):.2f}R",
        "Drawdown": f"{float(winner.get('max_drawdown_r', 0.0)):.2f}R",
        "Periodo": f"{data.get('first_candle', 'N/D')} a {data.get('last_candle', 'N/D')}",
    }


def _model_exit_research_ranking_row(item: object) -> dict[str, object]:
    row = dict(item or {})
    return {
        "Stop ATR": float(row.get("stop_factor", 0.0) or 0.0),
        "Alvo R": float(row.get("risk_reward", 0.0) or 0.0),
        "Trades": int(row.get("trades", 0) or 0),
        "Win rate": round(float(row.get("win_rate", 0.0) or 0.0) * 100.0, 2),
        "Profit factor": round(float(row.get("profit_factor", 0.0) or 0.0), 3),
        "Expectativa R": round(float(row.get("expectancy_r", 0.0) or 0.0), 4),
        "Resultado R": round(float(row.get("net_r", 0.0) or 0.0), 2),
        "Drawdown R": round(float(row.get("max_drawdown_r", 0.0) or 0.0), 2),
    }


def _model_exit_research_heatmap_spec() -> dict[str, object]:
    return {
        "height": 260,
        "mark": {"type": "rect"},
        "encoding": {
            ...
            "color": {
                ...
                "scale": {
                    "domainMid": 0,
                    "range": ["#b6404a", "#f4f1e8", "#16875c"],
                },
            },
            "tooltip": [
                {"field": "Stop ATR", "title": "Stop ATR"},
                {"field": "Alvo R", "title": "Alvo R"},
                {"field": "Resultado R", "title": "Resultado R"},
                {"field": "Trades", "title": "Trades"},
                {"field": "Profit factor", "title": "Profit factor"},
                {"field": "Drawdown R", "title": "Drawdown R"},
            ],
        },
    }
...
def _render_m3_suggested_alpha_research_table() -> None:
    st.markdown("#### Pesquisa historica do antigo Modelo 3 (M3)")
    st.caption(
        "Pesquisa nested com treino 60%, validacao 20% e holdout final 20%. "
        "USDCAD preservou vantagem e e o unico par certificado. O mesmo "
        "contrato foi habilitado nos outros sete pares em Demo por solicitacao "
        "do usuario, identificado como expansao sem certificacao individual."
    )
    payload = _load_m3_suggested_alpha_research_payload()
    if payload is None:
        st.info(
            "Pesquisa nested do M3 ainda nao encontrada neste computador."
        )
        return
    _render_plain_markdown_table(
...
def _render_m4_contextual_frontier_table() -> None:
    st.markdown("#### Liquidity Reclaim experimental - Modelo 4 (M4)")
    st.caption(
        "O candidato AUDUSD BUY apresentou a melhor evidencia fora da amostra "
        "disponivel: 100 trades, PF 1,468, holdout PF 2,121 e estresse 2,017. "
        "O mesmo contrato foi expandido aos oito pares somente em Demo; os "
        "outros sete nao possuem certificacao historica individual."
    )
    ...
    st.caption(
        "A linha AUDUSD preserva a evidencia original. As demais linhas exibem "
        "a expansao operacional sem copiar metricas historicas. Nenhuma conta "
        "real esta autorizada."
    )
...
def _render_scenario_runner_confirmation_audit(research: object) -> None:
    ...
    st.caption(
        "Visao de conferencia: para cada par, lista as 17 Alphas da maior "
...
def _lab_historical_confirmation_audit_rows(
    ...
    """Seleciona a melhor evidencia e completa as 17 Alphas por par."""
...
def main() -> None:
    ...
    favicon_path = Path(__file__).resolve().parent / "assets" / "traderia_icon.png"
    st.set_page_config(
        page_title=app_title or "TraderIA Local",
